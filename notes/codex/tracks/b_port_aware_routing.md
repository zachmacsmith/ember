...
Implementation evidence: the separate `propagating_port_construction.py` passes six targeted test groups. Its one supplied-state ER insertion recovers a valid 64-chain/218-qubit minor, preserving all current future frontiers and the input entry. It uses 806,721 insertion units plus 184,048 setup units under the normal 20-million/60-second allowance; the filter accounts for 782 units across nine path calls. Diagnostic wall is 0.776 seconds, with no timing comparison claimed. This is distinct from B010's deliberately truncated original trace. Source/test records and the full recovered minor are under `results/codex/track-b011-checks`; no B011 panel observations have been seen yet.
