...
B006's completed screen exactly preserves all nine final/partial chains and choices while removing repeated computation; reuse is retained without a wall-time speed claim. B007 is implemented separately in `factored/propagating_growth_construction.py`. Fifteen guarded targeted checks pass in `results/codex/track-b007-checks/attempt001`, including the actual triangle/C4 constructor with one promotion, the frozen-required-contact variant, nested local/global work accounting and rollback. Partial propagation never authorizes extra Q. The test manifest binds source and test bytes; no B007 development-panel output has been observed yet.
