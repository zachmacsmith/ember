...
void expanding_chain(Embedding* embedding, Graph* P, Pass* ps) {
    ...
    // QEBench fix: size to actual old_node count rather than magic [200]
...
int main(int argc, char *argv[]) {
    ...
    if (is_beginning == 0) {
        ...
        //embedding->print();
        // QEBench DIAGNOSTIC: skip expanding_border to test if cumulative
        // (+1,+1) shifting is causing the per-step protocol to wedge.
        // embedding->expanding_border();
        // QEBench fix: bound the per-call extend loop. Without this the
        // loop can spin forever when the virtual grid is too crowded for
        // find_embedding_own to ever succeed for the requested chain.
        // 2s wall-clock + 50-iteration cap; on cutoff we fall through
        // and write whatever embedding state exists so the Python wrapper
        // detects a no-progress (Δ+0) soft-fail.
        auto extend_begin = chrono::steady_clock::now();
        const double EXTEND_TIME_LIMIT_S = 2.0;
        const int    EXTEND_ITER_LIMIT   = 50;
        int extend_iter = 0;
        int prev_sz = (int)(embedding->get_selected()->size());
        int stagnant_iters = 0;
        while (true) {
            ++extend_iter;
            auto now = chrono::steady_clock::now();
            double elapsed = chrono::duration_cast<chrono::milliseconds>(
                                 now - extend_begin).count() / 1000.0;
            if (elapsed > EXTEND_TIME_LIMIT_S || extend_iter > EXTEND_ITER_LIMIT) {
                check_expand = -1;  // sentinel: aborted
                break;
            }
            ...
            // Detect no-progress: if expanding_chain + (later) expanding_total
            // makes no change for several iterations, give up.
            int curr_sz = (int)(embedding->get_selected()->size());
            if (curr_sz == prev_sz) {
                if (++stagnant_iters >= 3) { check_expand = -1; break; }
            } else {
                stagnant_iters = 0;
                prev_sz = curr_sz;
            }
