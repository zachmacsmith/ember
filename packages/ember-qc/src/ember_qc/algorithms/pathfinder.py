"""
ember_qc/algorithms/pathfinder.py
=================================
PathFinder — negotiated rip-up-and-reroute for minor embedding (approach 3.5).
...
1. **A real Steiner inner step.** MM rebuilds a vertex's chain as a *union of
   ...
   heuristic). PathFinder grows one tree with the shortest-path heuristic (SPH):
...
2. **Negotiated congestion (McMurchie–Ebeling 1995).** MM prices an over-used
   ...
   PathFinder prices qubit ``q`` by ``(1 + history[q]) · (1 + pres_fac · occ[q])``,
...
dense-graph chains balloon. So PathFinder's headline mode is an **improver**: it
...
Registered algorithms:
    pathfinder            MM-seeded negotiated improver (balanced)
    pathfinder-thorough   more rounds, deeper search — slower, shorter chains
    pathfinder-cold       standalone BFS cold start (no MM seed), then improve
...
class PathFinderRouter:
    """One PathFinder run over a fixed (source, target) pair.
    ...
    def _seed_embedding(self, timeout: float) -> bool:
        ...
        try:
            ...
            logger.debug("pathfinder base seed failed: %s", exc)
            return False
        if not raw:
            return False
...
def embed_pathfinder(
    ...
    **params,
    ...
    try:
        ...
        logger.error("pathfinder error: %s", exc)
...
class _PathFinderBase(EmbeddingAlgorithm):
