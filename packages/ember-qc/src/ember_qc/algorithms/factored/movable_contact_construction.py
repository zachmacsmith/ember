"""B019 standalone movable-contact constructor; standard library only.
...
class _Context:
    ...
    def partner(self, state, edge):
        ...
        candidates = set()
        for u in edge:
            for candidate in self.incident[u]:
                m.step()
                if candidate != edge:
                    candidates.add(candidate)
        if not candidates:
            return [edge]
        second = min(m.walk(candidates), key=lambda e: (-(state.h[e[0]]+state.h[e[1]]),
                         -(len(state.trees[e[0]])+len(state.trees[e[1]])), self.edge_rank[e]))
        return sorted((edge, second))
...
def contact_embed(source, target, *, timeout=60.0, deadline=None, seed=0):
    ...
    began = perf_counter()
    info = dict(algorithm='movable-contact-trees', version='B019', seed=seed,
