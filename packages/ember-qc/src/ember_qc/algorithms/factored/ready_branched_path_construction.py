"""One dependency-ready A053 construction followed by bounded retained-branch path moves."""
import math
import time

from ember_qc.algorithms.factored.ready_lifting_construction import reduced_core_embed as _base
...
def branched_path_embed(source, target, *, seed=0, timeout=60., deadline=None):
    ...
    info = dict(algorithm='ready_a053_retained_branch_path', version=1, base_calls=0,
                ...
                import_timing='fixed ready-A053 and path helpers import eagerly outside pilot solver time')
    current = None
    response = {}
    status = 'TIMEOUT'
    try:
