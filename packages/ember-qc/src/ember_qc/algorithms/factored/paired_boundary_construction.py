"""A066: one A061 construction and coordinated changes to its evolving state."""
import math
import time

from ember_qc.algorithms.factored.branched_path_construction import branched_path_embed as _base
from ember_qc.algorithms.factored.paired_boundary_reconstruction import (
    paired_boundary_reconstruction as _operator,
)
from ember_qc.algorithms.factored.mobile_boundary_reconstruction import (
    _Meter, _ReadMap, _ReadSource, _Stop,
)
from ember_qc.embedding_backend import _is_valid_embedding as _validator


def _run(source, target, *, seed=0, timeout=60., deadline=None, enable_pairs):
    ...
    info = dict(algorithm='a061_paired_boundary', version='A066', pair_policy=enable_pairs, base_calls=0,
...
def paired_boundary_embed(source, target, *, seed=0, timeout=60., deadline=None):
    return _run(source, target, seed=seed, timeout=timeout, deadline=deadline, enable_pairs=True)


def pruned_boundary_embed(source, target, *, seed=0, timeout=60., deadline=None):
    """Constant ablation: prove and close futile queries, without pair search."""
    return _run(source, target, seed=seed, timeout=timeout, deadline=deadline, enable_pairs=False)
