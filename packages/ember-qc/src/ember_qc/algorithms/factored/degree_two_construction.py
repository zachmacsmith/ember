"""Degree-two filled-core expansion with unchanged bounded one-site transfers.
...
def _reduce(engine):
    ...
    heap = [(len(row), engine.vrank[v], v) for v, row in work.items()
            if v not in anchors and len(row) <= 2]
    heapq.heapify(heap)
    journal = []
    while heap:
        engine.check()
        ...
        for w in neighbors:
            if w not in anchors and len(work[w]) <= 2:
...
def reduced_core_embed(source_graph, target_graph, *, seed=0, timeout=60.0, deadline=None):
    ...
    info = dict(algorithm='degree_two_core_expansion_site_transfer', version=4, seed=seed,
