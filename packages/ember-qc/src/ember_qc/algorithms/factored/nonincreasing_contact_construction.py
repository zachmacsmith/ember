"""B025: nonincreasing current-price acceptance in frozen B023 construction.
...
See notes/codex/tracks/b_025_acceptance_discriminator.md for the fixed policy.
"""
...
class _Context:
    ...
    def _action(self, kind, key, generate):
        ...
        row = dict(kind=kind, selected=key, before=self.state.score(), committed=False,
                   ...
                   acceptance_rejected=False, selected_proposal=None)
        ...
        try:
            ...
            if winner is not None:
                with m.phase('acceptance'):
                    m.step()
                    row['acceptance_threshold'] = self.state.F
                    row['selected_proposal'] = dict(
                        owner=winner['owner'],
                        sites=sorted(m.walk(winner['sites']), key=self.rank.__getitem__),
                        score=dict(m.walk(winner['score'].items())),
                        lost_contacts=list(m.walk(winner['lost'])),
                        restored_contacts=list(m.walk(winner['restored'])))
                    eligible = winner['score']['energy'] <= self.state.F
                    row['acceptance_eligible'] = eligible
                    m.clock()
                if eligible:
                    self.publish(winner, row)
                else:
                    row['acceptance_rejected'] = True
                    row['discarded_scored'] = row['scored']
                    self.stats['acceptance_rejections'] += 1
                    self.stats[kind+'_acceptance_rejections'] += 1
            else:
                m.clock()
            row['complete'] = True
        except _Stop as exc:
...
def contact_embed(source, target, *, timeout=20.0, deadline=None, seed=0):
    ...
    info = dict(algorithm='nonincreasing-contact-construction', version='B025', seed=seed,
