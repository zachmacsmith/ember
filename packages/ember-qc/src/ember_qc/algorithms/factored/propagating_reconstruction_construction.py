...
Standalone standard-library constructor, specified in b_blocked_reconstruction.md.
        ...
        try:
            ...
            for index, selected in enumerate(blocks):
                ...
                active = dict(index=index, selected=list(selected), order=[],
                              status='started', units=0, growth_indices=[])
            ...
            record['status'] = 'all_blocks_failed' if blocks else 'empty_pool'
            return None, record
        except _RepairLimit:
            record['status'] = 'local_work_limit'
            return None, record
...
def frontier_embed(source_graph, target_graph, *, timeout=60.0, deadline=None, seed=0):
    ...
    info = dict(algorithm='blocked_reconstruction_domain_trees', version=12, seed=seed,
