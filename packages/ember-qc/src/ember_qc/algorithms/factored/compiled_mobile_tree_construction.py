"""B028: unchanged B027 trajectory with compiled exact joint-root routing.

See notes/codex/tracks/b_028_routing_policy.md. Compilation is charged. Counters measure work;
...
class _Engine:
    ...
    def joint_root(self, neighbors, chains, occupancy, quality):
        ...
        if not hasattr(self, '_b028_kernel'):
            with self.m.phase('routing_import'):
                import importlib.util
                from pathlib import Path
                import sys
                path = Path(__file__).with_name('compiled_joint_root.py')
                ...
                if name not in sys.modules:
                    ...
                    try:
                        ...
                        raise
                self._b028_kernel = sys.modules[name]
        return self._b028_kernel.joint_root(self, neighbors, chains, occupancy, quality)
...
def contact_embed(source, target, *, timeout=60.0, deadline=None, seed=0):
    ...
    began = perf_counter()
    info = dict(algorithm='compiled-mobile-tree-construction', version='B028', seed=seed,
