"""Supported-degree-three reduction with unchanged A053 transfer expansion.
...
def _reduction_charge(engine, key):
    """Count added source work inside, never in addition to, global scans."""
    before = engine.scans
    try:
        engine.scan()
    finally:
        counts = engine.info['reduction_detail']['work']
        counts[key] = counts.get(key, 0) + engine.scans - before


def _eligible(engine, work, anchors, v):
    _reduction_charge(engine, 'eligibility_checks')
    if v in anchors:
        return False
    if len(work[v]) <= 2:
        return True
    ...
    for i, a in enumerate(neighbors):
        for b in neighbors[i+1:]:
            _reduction_charge(engine, 'neighbor_pair_checks')
            if b in work[a]:
                return True
    engine.info['reduction_detail']['degree3_rejections'] += 1
    return False


def _reduce(engine):
    """Return the complete supported filled core, or expose no partial core."""
    original = engine.source
    detail = dict(work={}, degree3_rejections=0, nonlocal_enqueues=0,
                  nonlocal_activations=0, stage='copy', current_row=None,
                  row_plan_complete=False, complete=False)
    engine.info['reduction_detail'] = detail
    ...
    detail['anchors'] = sorted(anchors)
    detail['stage'] = 'initial_heap'
    heap = []
    for v in work:
        if _eligible(engine, work, anchors, v):
            _reduction_charge(engine, 'queued_entries')
            heapq.heappush(heap, (len(work[v]), engine.vrank[v], v))
    # Preserve completed rows even if a later row/update is interrupted.
    journal = []
    engine.info['journal'] = journal
    while heap:
        detail['stage'] = 'heap_pop'
        engine.check()
        degree, _, v = heapq.heappop(heap)
        if v not in work or len(work[v]) != degree or not _eligible(engine, work, anchors, v):
            continue
        neighbors = tuple(sorted(work[v]))
        fill = []
        detail.update(stage='row_plan', current_row=dict(vertex=v, neighbors=neighbors,
                      created_fill=fill), row_plan_complete=False)
        for i, a in enumerate(neighbors):
            for b in neighbors[i+1:]:
                engine.scan()
                if b not in work[a]:
                    fill.append((a, b))
        detail.update(stage='row_mutation', row_plan_complete=True)
        for a, b in fill:
            work[a].add(b); work[b].add(a)
        for w in neighbors:
            engine.scan()
            work[w].remove(v)
        del work[v]
        journal.append({'vertex': v, 'neighbors': neighbors, 'created_fill': tuple(fill)})
        detail.update(stage='affected_update', current_row=None)
        affected = set()
        for w in neighbors:
            _reduction_charge(engine, 'affected_members')
            affected.add(w)
        for a, b in fill:
            a, b = sorted((a, b), key=lambda q: (len(work[q]), q))
            for w in sorted(work[a]):
                _reduction_charge(engine, 'common_neighbor_scans')
                if w in work[b]:
                    affected.add(w)
        added = set(fill)
        for w in sorted(affected, key=engine.vrank.__getitem__):
            if _eligible(engine, work, anchors, w):
                if w not in neighbors:
                    detail['nonlocal_enqueues'] += 1
                    if len(work[w]) == 3:
                        old_support = False
                        ns = sorted(work[w])
                        for i, a in enumerate(ns):
                            for b in ns[i+1:]:
                                _reduction_charge(engine, 'activation_pair_checks')
                                if b in work[a] and (a, b) not in added:
                                    old_support = True
                        if not old_support:
                            detail['nonlocal_activations'] += 1
                _reduction_charge(engine, 'queued_entries')
    ...
    engine.check()
    detail.update(stage='complete', complete=True, current_row=None)
...
def reduced_core_embed(source_graph, target_graph, *, seed=0, timeout=60.0, deadline=None):
    ...
    info = dict(algorithm='supported_degree_three_core_expansion_site_transfer', version=5, seed=seed,
