...
Standalone standard-library constructor, specified in b_frontier_construction.md.
    ...
    def prices(self, chains, owners, bounds, free):
        ...
        def price(q):
            if q not in cache:
                ...
                cache[q] = 1. + sum(weights[v] for v in touching)
            return cache[q]
        return price
...
def frontier_embed(source_graph, target_graph, *, timeout=60.0, deadline=None, seed=0):
    ...
    info = dict(algorithm='joint_frontier_trees', version=2, seed=seed,
