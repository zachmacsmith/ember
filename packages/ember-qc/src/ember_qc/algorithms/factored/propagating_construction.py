...
Standalone standard-library constructor, specified in b_domain_propagation.md.
...
class _Builder(_Engine):
    ...
    def singleton_domains(self, chains, promoted):
        self.check()
        ...
        for u in self.source:
            ...
            for w in self.source[u]:
                ...
                if w in chains:
                    mask &= bounds[w]
            domains[u] = mask
        return self.arc_queue(domains, free)[0]
...
def frontier_embed(source_graph, target_graph, *, timeout=60.0, deadline=None, seed=0):
    ...
    info = dict(algorithm='propagating_connected_trees', version=5, seed=seed,
