"""B028's local exact B027 query, compiled without disk caching.
...
def _target(engine):
    if hasattr(engine, '_b028_target'):
        return engine._b028_target
    m = engine.m
    with m.phase('routing_target_pack'):
        ...
        m.clock()
        engine._b028_target = result
        return result


def _acceleration(engine):
    if not hasattr(engine, 'acceleration'):
        engine.acceleration = dict(version='B028', queries=0, completed_queries=0,
...
def joint_root(engine, neighbors, chains, occupancy, quality):
    ...
    try:
        ...
        before = perf_counter()
        try:
            with m.phase('routing_conversion'):
                final_distance, final_parent = {}, {}
                for j, v in enumerate(neighbors):
                    distances_v, parents_v = {}, {}
                    for i in range(int(settlement_counts[j])):
                        q = int(settlement_order[j, i])
                        distances_v[engine.qorder[q]] = int(settled[j, q])
                        m.step('compiled_settled_unpack')
                    for i in range(int(discovery_counts[j])):
                        q = int(discovered[j, i])
                        predecessor = int(parent[j, q])
                        if predecessor == UNKNOWN_PARENT:
                            raise RuntimeError('discovered site has unknown parent')
                        parents_v[engine.qorder[q]] = (None if predecessor == NO_PARENT
                                                       else engine.qorder[predecessor])
                        m.step('compiled_parent_unpack')
                    final_distance[v], final_parent[v] = distances_v, parents_v
                result = (engine.qorder[int(control[1])], final_distance, final_parent,
                    dict(root_score=int(control[0]), labels=int(work[2]), pushes=int(work[0]),
