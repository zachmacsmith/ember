"""
ember_qc/algorithms/pf_bounded.py
=================================
PathFinder variant **S3 — early-termination + bounded routing region**.

ISOLATED experiment (see docs/candidate-algorithms/pf-improvements/bounded.md).
The frozen baseline ``pathfinder`` rebuilds every chain in ``_steiner_route`` by
...
Registered algorithms:
    pathfinder-bounded        region + early-termination (recommended)
    pathfinder-bounded-region region only (ablation: no targets early-stop)
...
from ember_qc.algorithms.pathfinder import (
    PathFinderRouter,
    _PathFinderBase,
    embed_pathfinder,
    _BIG,
...
class PathFinderBoundedRouter(PathFinderRouter):
    """PathFinder router with a bounded, early-terminating Steiner inner step.

    Adds four knobs on top of :class:`PathFinderRouter`; everything else (seeding,
    ...
    def _steiner_route(
        ...
        Same contract as :meth:`PathFinderRouter._steiner_route`: returns a
...
# PathFinderBoundedRouter is a production optimization component: it is composed
# into the optimized PathFinder router in pathfinder_opt.py (no standalone
