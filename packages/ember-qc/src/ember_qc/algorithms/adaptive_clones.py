"""C010: conflict-directed clone splitting with bounded greedy placement.
...
class _Budget:
    ...
    def tick(self, name, amount=1):
        if self.units + amount > self.limit:
            self.denied = dict(name=name, amount=amount)
            raise _Stop('work')
...
def clone_embed(source, target, *, seed=0, timeout=15., deadline=None):
    ...
    response = dict(status='FAILURE', embedding={}, diag=dict(
        algorithm='adaptive_clones', policy='C010', visits=[], fatal_error=False,
        source_label_order='supplied node order; diagnostic clone/target IDs are ranks',
        search_limit=19_000_000, total_limit=20_000_000, final_reserve=0.5))
