...
numba-compiled node-weighted Dijkstra for PathFinder routing, tested two ways:
  ...
  * over the BOUNDED region with a persistent +inf cost array so per-route setup
    ...
    `pathfinder` recipe but with the compiled kernel) -> a WASH, within noise of
    pure-Python `pathfinder` (e.g. n40 d0.7 Pegasus-6: 1.74s vs 1.74s; identical
    or marginally better ACL; valid; deterministic).

Conclusion: compiling the routing inner loop does not speed up the optimized
PathFinder. Bounded-region routing has already shrunk each Dijkstra to a small
...
(70-84%, profiled). PathFinder's wall-clock is >= minorminer's by construction
...
Kept (un-registered, importable via `import ember_qc.algorithms.pf_numba`) only as
the reproducible artifact behind the paper's Limitations finding. Registers the
`pathfinder-numba` algorithm on import for measurement; `__init__.py` does NOT
...
class NumbaBoundedRouter(PathFinderBoundedRouter):
    ...
    def _steiner_route(self, v, cost, forbidden_extra=None):
        ...
        return PathFinderRouter._steiner_route(self, v, cost, forbidden_extra)
...
@register_algorithm("pathfinder-numba")
class PathFinderNumba(_PathFinderBase):
