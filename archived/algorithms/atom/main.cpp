...
void initialize_embedding(Embedding *eb, vector<int> *seed_set,
                          int seed_limit) {
  int c_count = 0;
...
void extract_order(Graph *P, vector<Pass *> *list_passes, vector<int> *seed_set,
                   int seed_limit) {
  int n = P->get_n();
  vector<int> orders;
  while (true) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (it == seed_set->end()) {
        for (auto seed_it = seed_set->begin(); seed_it != seed_set->end();
             ++seed_it) {
          if (P->has_edge(i, *seed_it)) {
            countt =
                countt - (double)(distance(seed_set->begin(), seed_it)) + n;
            // countt = countt + 1;
          }
        }
        // cout<<countt<<' ';
  ...
  // QEBench fix: guard against erasing from empty vector (original UB)
...
void expanding_chain(Embedding *embedding, Graph *P, Pass *ps) {
  int n = P->get_n(), topo_row = embedding->get_topo_row(),
  ...
  for (auto it = old_node->begin(); it != old_node->end(); ++it) {
    ...
    if (gap > -3) {
      for (auto p_it = free_neighbors[idx].begin();
           p_it != free_neighbors[idx].end(); ++p_it) {
        int x = (*p_it)->x, y = (*p_it)->y, k = (*p_it)->k;
...
void BFS(Embedding *embedding, int it, int idx, int topo_row, int topo_column,
         int *label_HW, int *f) {
  /*long long dem = 0;
  ...
  while (!q->empty()) {
    ...
    q->pop();
    vector<Point *> *neighbors =
        head->get_neighbors(-1, -1, topo_row, topo_column);
    for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
      int x = (*it)->x, y = (*it)->y, k = (*it)->k;
      if (visited[x * topo_column * 8 + y * 8 + k] == -1 &&
          label_HW[x * topo_column * 8 + y * 8 + k] == -1) {
        visited[x * topo_column * 8 + y * 8 + k] =
            visited[head->x * topo_column * 8 + head->y * 8 + head->k] + 1;
        f[idx * topo_row * topo_column * 8 + x * topo_column * 8 + y * 8 + k] =
            visited[x * topo_column * 8 + y * 8 + k];
...
pair<int, int> calculate_DP(Tree_Point *curr_root) {
  ...
  for (auto it = curr_root->children->begin(); it != curr_root->children->end();
       ++it) {
...
Tree_Point *find_dynamic_tree(Point *good_pos, int topo_row, int topo_column,
                              int *label_HW, vector<int> *old_node, int n) {
  queue<Tree_Point *> q;
  int *visited = new int[topo_row * topo_column * 8]();
  for (int i = 0; i < topo_row; ++i) {
    for (int j = 0; j < topo_column; ++j) {
  ...
  while (!q.empty()) {
    ...
    for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
      ...
      if (visited[x * topo_column * 8 + y * 8 + k] == -1) {
        ...
        if (color_tail >= 0) {
          if (find(old_node->begin(), old_node->end(), color_tail) !=
                  old_node->end() &&
              !check_eb[color_tail]) {
...
bool is_good_pos(Point *A, Point *B) { return (A->x == B->x && A->y == B->y); }

void allocate_node(Tree_Point *curr_root, int new_color, Embedding *embedding,
                   bool check_first, bool check_alone, int curr_depth,
                   Graph *P) {
  ...
  if (curr_root->numOfdes > 1 || check_first) {
    EB_Point *new_embed =
        new EB_Point(curr_root->x, curr_root->y, curr_root->k, new_color);
    embedding->append(new_embed);
  } else {
    int old_color = curr_root->expected_color;
    if (check_alone) {
      EB_Point *new_embed =
          new EB_Point(curr_root->x, curr_root->y, curr_root->k, old_color);
      embedding->append(new_embed);
    } else {
      if (P->get_degree(old_color) - curr_depth < 0) {
        ...
        int rand_val =
            rand() % (P->get_degree(old_color) + P->get_degree(new_color));
        if (rand_val < P->get_degree(old_color) ||
            is_good_pos(curr_root, curr_root->parent)) {
          EB_Point *new_embed =
              new EB_Point(curr_root->x, curr_root->y, curr_root->k, old_color);
          embedding->append(new_embed);
          chek_alone = true;
  ...
  for (auto it = curr_root->children->begin(); it != curr_root->children->end();
       ++it) {
    allocate_node(*it, new_color, embedding, false, chek_alone, curr_depth + 1,
                  P);
  }
  return;
}

void allocate_node_test(Tree_Point *curr_root, int new_color,
                        vector<EB_Point *> *new_selected, bool check_first,
                        bool check_alone, int curr_depth, Graph *P) {
  bool chek_alone = check_alone;
  if (curr_root->numOfdes == 0 || curr_root->color != -1) {
    return;
  }
  if (curr_root->numOfdes > 1 || check_first) {
    ...
    if (check_alone) {
      ...
      if (P->get_degree(old_color) - curr_depth < 0) {
        ...
        int rand_val = rand() % (P->get_degree(old_color) +
                                 P->get_degree(new_color) - curr_depth + 1);
        if (rand_val < P->get_degree(old_color) ||
            is_good_pos(curr_root, curr_root->parent)) {
          EB_Point *new_embed =
              new EB_Point(curr_root->x, curr_root->y, curr_root->k, old_color);
  ...
  for (auto it = curr_root->children->begin(); it != curr_root->children->end();
       ++it) {
    allocate_node_test(*it, new_color, new_selected, false, chek_alone,
                       curr_depth + 1, P);
  }
  return;
}
...
bool find_embedding_own(Embedding *embedding, Graph *P, Pass *ps) {
  ...
  // QEBench fix: use serial BFS instead of buggy threaded version.
  // The original threading code segfaults due to incorrect std::thread
  // parameter passing. The authors included this serial fallback.
  int idx = 0;
  for (auto it = old_node->begin(); it != old_node->end(); ++it) {
  ...
  for (int i = 0; i < topo_row; ++i) {
    for (int j = 0; j < topo_column; ++j) {
      for (int k = 0; k < 8; ++k) {
        ...
        for (auto it = old_node->begin(); it != old_node->end(); ++it) {
          if (label_HW[i * topo_column * 8 + j * 8 + k] >= 0 ||
              f[idx * topo_row * topo_column * 8 + i * topo_column * 8 + j * 8 +
                k] == -1) {
            check_embedded = false;
            break;
          } else {
            sum_embedded += f[idx * topo_row * topo_column * 8 +
                              i * topo_column * 8 + j * 8 + k];
            //                        sum_f[i*topo_row*8+j*8+k] +=
            //                        f[idx*topo_row*topo_column*8 +
            //                        i*topo_column*8 + j*8 + k];
          }
          idx = idx + 1;
        }
  ...
  // cout << "Good Pos: " <<good_pos->x<< ' ' << good_pos->y << '
  // '<<good_pos->k<<' '<<maxn<<endl;
  ...
  for (int i = 0; i < topo_row; ++i) {
      for (int j = 0; j < topo_column; ++j) {
          for (int k = 0; k < 8; ++k) {
              ...
              if (label_HW [idx] == -1 && check_f[idx] && sum_f[idx] >= maxn -
  epsilon) { Point* estimate_pos = new Point(); estimate_pos->x = i;
                  estimate_pos->y = j;
                  estimate_pos->k = k;
                  Tree_Point* root = find_dynamic_tree(estimate_pos, topo_row,
  topo_column, label_HW, old_node, n); vector<EB_Point*>* selected =
  embedding->get_selected(); vector<EB_Point*>* new_selected = new
  vector<EB_Point*>(); for (auto it = selected->begin(); it != selected->end();
  it++) { new_selected->push_back(new EB_Point((*it)->x, (*it)->y, (*it)->k,
  (*it)->color));
                  }
                  bool check_first = true, check_alone = false;
                  int curr_depth = 0;
                  allocate_node_test(root, new_color, new_selected, check_first,
  check_alone, curr_depth, P); if (i == good_pos->x && j == good_pos->y && k ==
  good_pos->k) { cout << i<< ' ' <<j<<' '<<k<<' '<<new_selected->size() << endl;
                  }
  ...
  if (good_pos->x == -1) {
    ...
    Tree_Point *root = find_dynamic_tree(good_pos, topo_row, topo_column,
                                         label_HW, old_node, n);
    bool check_first = true, check_alone = false;
    int curr_depth = 0;
    allocate_node(root, new_color, embedding, check_first, check_alone,
                  curr_depth, P);
    delete_tree(root);
    delete good_pos;
    delete[] label_HW;
...
void refresh(queue<Point *> *q, int *visited, Point *tail,
             vector<Point *> *cardinity, int topo_row, int topo_column,
             int limit_visited, const int num_HW) {
  while (!q->empty()) {
    q->pop();
  }
  Point *head = new Point(tail->x, tail->y, tail->k);
  while (visited[head->x * topo_column * 8 + head->y * 8 + head->k] !=
         num_HW * limit_visited) {
    cardinity->push_back(new Point(head->x, head->y, head->k));
    vector<Point *> *neighbors =
        head->get_neighbors(-1, -1, topo_row, topo_column);
    for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
      int x = (*it)->x, y = (*it)->y, k = (*it)->k;
      if (visited[x * topo_column * 8 + y * 8 + k] ==
          visited[head->x * topo_column * 8 + head->y * 8 + head->k] - 1) {
        head = *it;
        break;
      }
...
bool BFS_TST(vector<Point *> *cardinity, int topo_row, int topo_column,
             int *label_HW, vector<int> *old_node, int n, int idx) {
  queue<Point *> *q = new queue<Point *>();
  int *visited = new int[topo_row * topo_column * 8];
  for (int i = 0; i < topo_row; ++i) {
    for (int j = 0; j < topo_column; ++j) {
  ...
  while (!q->empty()) {
    ...
    for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
      ...
      if (label_HW[x * topo_column * 8 + y * 8 + k] >= 0 &&
          label_HW[head->x * topo_column * 8 + head->y * 8 + head->k] >= 0) {
        continue;
      }
      if (visited[x * topo_column * 8 + y * 8 + k] < limit_visited * num_HW) {
        if (label_HW[x * topo_column * 8 + y * 8 + k] == -1) {
          ...
          if (dx_old[label_HW[x * topo_column * 8 + y * 8 + k]]) {
            ...
            refresh(q, visited, head, cardinity, topo_row, topo_column,
                    limit_visited, num_HW);
...
bool find_embedding_2(Embedding *embedding, Graph *P, Pass *ps) {
  ...
  for (auto it = old_node->begin(); it != old_node->end(); ++it) {
    ...
    check = BFS_TST(cardinity, topo_row, topo_column, label_HW, old_node,
                    P->get_n(), *it);
...
bool test_chain_connection(Embedding *embedding, Graph *P) {
  int n = P->get_n();
  int topo_row = embedding->get_topo_row(),
      topo_column = embedding->get_topo_column();
  int *label_HW = embedding->generate_labelHW();
  vector<EB_Point *> *selected = embedding->get_selected();
  int nums[n];
    ...
    if (dx[x][y][k]) {
      ...
      while (!q.empty()) {
        ...
        for (auto it_p = neighbors->begin(); it_p != neighbors->end(); ++it_p) {
          ...
          if (label_HW[xp * topo_column * 8 + yp * 8 + kp] == color &&
              !dx[xp][yp][kp]) {
      ...
      if (P->has_edge(i, j)) {
        ...
        for (auto it = selected->begin(); it != selected->end(); it++) {
          ...
          if (color == i || color == j) {
            vector<Point *> *neighbors =
                (*it)->get_neighbors(-1, -1, topo_row, topo_column);
            for (auto it_p = neighbors->begin(); it_p != neighbors->end();
                 ++it_p) {
              int xp = (*it_p)->x, yp = (*it_p)->y, kp = (*it_p)->k,
                  colorp = label_HW[xp * topo_column * 8 + yp * 8 + kp];
  ...
  for (int i = 0; i < se_size; ++i) {
    ...
    for (int j = i + 1; j < se_size; ++j) {
      ...
      if (p1->x == p2->x && p1->y == p2->y && p1->k == p2->k) {
        cout << "Wrong at: (" << p1->x << ',' << p1->y << ',' << p1->k << ','
             << p1->color << "), (" << p2->x << ',' << p2->y << ',' << p2->k
             << ',' << p2->color << ')' << endl;
        total_check = false;
      }
    }
...
bool test_chain_connection_lightweight(int n, Embedding *embedding,
                                       EB_Point *removed_point, int *label_HW) {
  int nums = 0, topo_row = embedding->get_topo_row(),
      topo_column = embedding->get_topo_column();
  vector<EB_Point *> *selected = embedding->get_selected();
  ...
  dx->push_back(new EB_Point(saved_node->x, saved_node->y, saved_node->k,
                             saved_node->color));
  ...
  while (idx < dx->size()) {
    ...
    for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
      int x = (*it)->x, y = (*it)->y, k = (*it)->k;
      if (label_HW[x * topo_column * 8 + y * 8 + k] == color &&
          !check_exist(dx, *it)) {
...
bool test_global_connection_lightweight(Embedding *embedding,
                                        EB_Point *removed_point, int *label_HW,
                                        Graph *P) {
  vector<EB_Point *> *selected = embedding->get_selected();
  int color = removed_point->color, topo_row = embedding->get_topo_row(),
      topo_column = embedding->get_topo_column();
  vector<Point *> *neighbors =
      removed_point->get_neighbors(-1, -1, topo_row, topo_column);
  vector<int> *check_colors = new vector<int>();
  for (auto it = neighbors->begin(); it != neighbors->end(); ++it) {
    int x = (*it)->x, y = (*it)->y, k = (*it)->k;
    int neighbor_color = label_HW[x * topo_column * 8 + y * 8 + k];
    if (neighbor_color != -1 && neighbor_color != color &&
        P->has_edge(color, neighbor_color)) {
  ...
  for (auto it = selected->begin(); it != selected->end(); it++) {
    ...
    if (label_HW[x * topo_column * 8 + y * 8 + k] == removed_point->color) {
      ...
      for (auto it_p = neighbors->begin(); it_p != neighbors->end(); ++it_p) {
        ...
        if (label_HW[x * topo_column * 8 + y * 8 + k] != -1 &&
            label_HW[x * topo_column * 8 + y * 8 + k] != color) {
          auto it_check = find(check_colors->begin(), check_colors->end(),
                               label_HW[x * topo_column * 8 + y * 8 + k]);
...
void remove_trash(int n, Embedding *embedding, int last_selected_size,
                  Graph *P) {
  vector<EB_Point *> *selected = embedding->get_selected();
  ...
  while (it != selected->end()) {
    ...
    label_HW[x * topo_column * 8 + y * 8 + k] = -1;
    if (!test_chain_connection_lightweight(n, embedding, *it, label_HW) ||
        !test_global_connection_lightweight(embedding, *it, label_HW, P)) {
...
void expanding_vertical(int pivot, vector<int> *old_node, Embedding *embedding,
                        Graph *P) {
  int topo_row = embedding->get_topo_row(),
      topo_column = embedding->get_topo_column();
  vector<EB_Point *> *selected = embedding->get_selected();
    ...
    if ((*it)->x > pivot) {
      ...
      if ((*it)->x == pivot) {
        if ((*it)->k <= 3) {
          ...
          int co = -1;
          if (P->has_edge(prev_co, next_co) ||
              (prev_co == next_co && prev_co != -1)) {
            if (find(old_node->begin(), old_node->end(), prev_co) !=
                old_node->end()) {
              co = prev_co;
            } else {
              if (find(old_node->begin(), old_node->end(), next_co) !=
                  old_node->end()) {
                co = next_co;
              } else {
                if (rand() %
                        (P->get_degree(prev_co) + P->get_degree(next_co) + 1) <
                    P->get_degree(prev_co)) {
                  co = prev_co;
                } else {
                  co = next_co;
                }
...
void expanding_horizontal(int pivot, vector<int> *old_node,
                          Embedding *embedding, Graph *P) {
  int topo_row = embedding->get_topo_row(),
      topo_column = embedding->get_topo_column();
  vector<EB_Point *> *selected = embedding->get_selected();
  int *label_HW = embedding->generate_labelHW();
...
void expanding_total(Embedding *embedding, Pass *ps, Graph *P) {
  ...
  for (auto it = selected->begin(); it != selected->end(); ++it) {
    if ((*it)->k > 3) {
      if ((*it)->x < r - 1 && find(r_select->begin(), r_select->end(),
                                   (*it)->x) == r_select->end()) {
        r_select->push_back((*it)->x);
      }
    } else {
      if ((*it)->y < c - 1 && find(c_select->begin(), c_select->end(),
                                   (*it)->y) == c_select->end()) {
...
int main(int argc, char *argv[]) {
  ...
  while (it != list_passes->end()) {
    ...
    for (auto it_p = (*it)->get_old_node()->begin(); it_p !=
    (*it)->get_old_node()->end(); ++it_p) { cout << (*it_p) << ' ';
    }
    cout<<endl;*/
    ...
    if (!find_embedding_own(embedding, P, *it)) {
      ...
      // embedding->print();
      for (auto it_p = (*it)->get_old_node()->begin();
           it_p != (*it)->get_old_node()->end(); ++it_p) {
