...
searching at solve time. The cache of problems Reweave has already solved is a
ready-made training set: each \emph{(problem graph, Reweave-optimized embedding)}
...
the non-neural \texttt{learned-retrieve} embeddings-cache, and the label-free
...
patent's premise is thus partly borne out: a learned model yields a small, real
...
per-instance portfolio cut both ACL and variance.}
\label{tab:mmfork}
\small
\begin{tabular}{lccc}
\toprule
algorithm & ACL (std) & vs.\ MM & vs.\ Reweave \\
\midrule
\texttt{minorminer} $\equiv$ \texttt{mmfork} & 3.641 (0.115) & --- & $+2.4\%$ \\
\texttt{mmfork-cuthill}    & 3.592 (0.105) & $-1.6\%$ & $+0.8\%$ \\
\texttt{reweave}           & 3.553 (0.102) & $-2.3\%$ & --- \\
\textbf{\texttt{mmfork-portfolio}} & \textbf{3.456} (\textbf{0.059}) & $\mathbf{-5.3\%}$ & $\mathbf{-3.1\%}$ \\
\bottomrule
\end{tabular}
\end{table}
