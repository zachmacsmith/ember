...
order-guided variants compose with it directly: seeding Reweave from
\texttt{mmfork-portfolio} rather than stock \texttt{minorminer} stacks two
independent gains---a better-ordered base \emph{and} the negotiated
destroy-repair improver---and yields the lowest ACL we observe
(Table~\ref{tab:stack}). The levers are orthogonal: ordering improves the base, and
Reweave's improver improves whatever base it is given. This is the same composition
mechanism as the existing \texttt{reweave-stacked} variant (which seeds from the
multilevel placement); \texttt{mmfork-portfolio} is simply a stronger, always-valid
base.
...
the five orders plus the default, so \texttt{mmfork-portfolio} is $5.9\times$
\texttt{minorminer}, and the stacked \texttt{reweave+mmfork} is $6.4\times$---the
improver adds little on top of the portfolio base, which is already near-optimal so
few destroy-repair moves are accepted (cf.\ standalone \texttt{reweave}'s
$1.7\times$). This is the same wall-clock regime as \texttt{reweave-thorough}
(\S\ref{sec:results}): a ${\sim}6\times$ constant factor bought for the lowest ACL
and the lowest variance, while the single-order variants stay at
\texttt{minorminer}'s cost. Table~\ref{tab:fgtime} gives per-cell seconds; all
algorithms run back-to-back on each instance so the ratios are robust to CPU
contention.
