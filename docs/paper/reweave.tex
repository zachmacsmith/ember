...
penalty. We package these as Reweave, an anytime, large-neighbourhood
...
success on every target), \texttt{reweave} never returns a longer-ACL
...
faster at equal-or-better ACL: single-seed \texttt{reweave} runs within
...
Reweave's outputs as training data, we ask whether an \emph{amortized} model can
learn to embed. The obvious supervised approach is ill-posed---hardware placement is
free under the fabric's symmetries, so the model collapses to a constant---and a
symmetry-invariant (Procrustes) objective fixes it, beating \texttt{minorminer} by
${\sim}1\%$ at equal budget (statistically significant but small); a decode-aware
reinforcement-learning fine-tune does not exceed it. A strong heuristic with restarts
thus remains hard to beat with learned placement---a cautionary result we report in
full, alongside a retraction of an earlier unequal-budget comparison.
\end{abstract}
...
(\S\ref{sec:reweave}). (ii) We package them as \textbf{Reweave}, an
anytime, large-neighbourhood \emph{improver} that warm-starts from a fast base
embedding and, by construction, never regresses below it. (iii) We evaluate Reweave
...
equal-or-better ACL, bringing single-seed \texttt{reweave} to within
${\sim}1.3\times$ compiled \texttt{minorminer}'s wall-clock. (v) Finally, we ask
whether \emph{learning} helps (\S\ref{sec:learning}) and report an honest, cautionary
answer: the naive supervised model is ill-posed and collapses, a symmetry-invariant
(Procrustes) objective fixes it and beats \texttt{minorminer} by ${\sim}1\%$ at equal
budget (small but significant), decode-aware RL does not exceed it, and we retract an
earlier unequal-budget comparison that had appeared to beat Reweave. We are explicit
...
\S\ref{sec:learning} learned a better \emph{initialization}; this section asks the
complementary question: can we guide \texttt{minorminer}'s \emph{search} itself?
Any \texttt{minorminer}-style embedder exposes two such levers---the \textbf{vertex
order} in which chains are built and rebuilt, and the \textbf{rip-up policy} that
chooses which chain to tear out and reroute next. \texttt{minorminer}'s own authors
named initial placement and vertex order as the key open problem, and much of its
run-to-run variance is order-driven. We test both levers, deterministically and
...
\begin{table}[t]
\caption{Stacking: seeding Reweave from \texttt{mmfork-portfolio}
(\texttt{reweave+mmfork}) combines the ordering and improver gains, beating both
its parents and stock \texttt{minorminer} (ER into $P_6$, mean ACL over $5$ seeds;
all methods $5/5$ valid).}
\label{tab:stack}
\small
\begin{tabular}{lcccc}
\toprule
cell & \texttt{minorminer} & \texttt{reweave} & \texttt{mmfork-portf.} & \texttt{reweave+mmfork} \\
\midrule
$n{=}40,d{=}0.5$ & 4.57 & 4.45 & 4.38 & \textbf{4.32} \\
$n{=}60,d{=}0.5$ & 7.71 & 7.30 & 6.89 & \textbf{6.62} \\
\bottomrule
\end{tabular}
\end{table}
...
\dots,10\}$ give identical ACL \emph{and} wall-clock---the single fixed order is
already as fast as the search gets. Table~\ref{tab:fgtime} gives per-cell seconds;
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\linewidth, height=5.2cm, xmode=log, log basis x=2,
  xtick={40,80,160,320}, xticklabels={40,80,160,320},
  xlabel={source size $n$ ($6$-regular, into Advantage2 $Z_{15}$)},
  ylabel={wall-clock $/$ \texttt{minorminer}}, ymin=0, ymax=8,
  legend pos=north east, legend style={font=\scriptsize, fill opacity=0.85},
  grid=both, grid style={gray!18}]
\addplot+[thick,mark=*] coordinates {(40,1)(80,1)(160,1)(320,1)}; \addlegendentry{minorminer}
\addplot+[thick,mark=triangle*] coordinates {(40,1.3)(80,1.0)(160,0.8)(320,0.6)}; \addlegendentry{mmfork-cuthill}
\addplot+[thick,mark=square*] coordinates {(40,1.7)(80,1.2)(160,1.3)(320,0.9)}; \addlegendentry{reweave+cuthill}
\addplot+[thick,mark=diamond*] coordinates {(40,4.9)(80,4.4)(160,3.3)(320,2.6)}; \addlegendentry{reweave-thorough}
\addplot+[thick,mark=otimes*] coordinates {(40,7.3)(80,6.6)(160,5.9)(320,4.4)}; \addlegendentry{mmfork-portfolio}
\end{axis}
\end{tikzpicture}
\caption{Wall-clock relative to \texttt{minorminer} ($=1$) versus source size $n$,
$6$-regular sources into Advantage2 $Z_{15}$ ($7440$ qubits, $5\%$ faults). Every
guided method's multiplier \emph{shrinks} with $n$; \texttt{mmfork-cuthill} and
\texttt{reweave-mmfork-cuthill} fall \emph{below} $1$ (faster than stock
\texttt{minorminer}) on the largest problems. $P_{16}$ behaves the same way.}
\label{fig:scaling}
\end{figure}
...
the swept sizes---by up to ${\sim}10\%$ at the densest feasible cell---though with a
single source instance per size the per-$n$ gains are noisy and not monotone, so we
do not claim the ACL advantage \emph{widens} with $n$, only that it persists. The
...
further substantiate the variance claims that are Reweave's headline advantage.
...
running them as a warm-started anytime improver, yields Reweave---a drop-in
...
single-seed \texttt{reweave} to within ${\sim}1.3\times$ compiled
...
ask whether a learned model can do better, and report an honest, cautionary result
(\S\ref{sec:learning}): the naive supervised approach is ill-posed and collapses to a
constant, a symmetry-invariant objective repairs it and beats \texttt{minorminer} by a
small but significant ${\sim}1\%$ at equal budget, and decode-aware reinforcement
learning does not exceed it---so a good heuristic with restarts remains hard to beat
with learned placement. The primitives are classical; the
