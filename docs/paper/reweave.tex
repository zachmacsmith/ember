...
up to $76\%$), all at $100\%$ success. An ablation attributes the gain honestly:
the two imported ingredients are individually interchangeable and only jointly
load-bearing---the warm-started anytime \emph{improver structure} itself does most
of the work. A structured optimization pass---bounded
...
the vertex order to its full search. A per-instance \emph{portfolio} of
bandwidth-reducing orders lowers \texttt{minorminer}'s ACL by ${\sim}5\%$ and
roughly halves its run-to-run variance; a single Cuthill--McKee order captures a
smaller gain at essentially zero cost. The guidance composes with Reweave
...
stays cheap at scale: averaged over instances, its wall-clock premium does not grow
with problem size, the single fixed order falling to parity with \texttt{minorminer}
on the largest Zephyr instances.
...
embedded random-Ising instances, shorter-chain embeddings give measurably better
solutions---\emph{within a fixed problem}, ACL anti-correlates with ground-state
probability (fixed-effects Spearman $\rho=-0.11$, $p=3{\times}10^{-8}$; the pooled
$\rho=-0.63$ additionally reflects problem difficulty), simulated annealing and
spin-vector Monte Carlo agree, and the lowest-ACL method is significantly better
paired per problem---and we re-establish the headline claims under a
$K{=}15$-instance redesign with confidence intervals and Holm-corrected paired
significance tests. The algorithmic
...
${\sim}1.3\times$ compiled \texttt{minorminer}'s wall-clock; a $2{\times}2$
ablation then attributes the quality gain, finding the two imported routing ideas
individually interchangeable and the improver structure the primary driver.
...
into its full search: a per-instance portfolio of orders lowers its ACL by
${\sim}5\%$ and roughly halves its seed variance (a single Cuthill--McKee order
gives a smaller gain for free), composes with Reweave
...
find the order-guided variants' wall-clock premium stays \emph{bounded} as problem
size grows (the single fixed order reaching parity with \texttt{minorminer} on the
largest Zephyr instances), while \texttt{mmfork-portfolio} holds a $5$--$7\%$ ACL
edge across scales.
...
embeddings yield measurably better annealing solutions at fixed problem
(fixed-effects $\rho=-0.11$, $p=3{\times}10^{-8}$; the mechanism---chain
breaks---and the method ranking agree across simulated annealing and spin-vector
Monte Carlo), and a $K{=}15$-instance redesign with confidence intervals
...
overlaps---then re-routes every chain it displaced through free space. (One
scoping note for precision: when the base embedding is valid, no qubit is ever
over-subscribed \emph{between} moves, so the history term never accumulates and
Eq.~\eqref{eq:cost} reduces to its present-occupancy factor inside the improver;
the history memory earns its keep in the cold-start and legalization paths, where
overlap persists across passes.) A move is
...
inner step that costs the most?}---which \S\ref{sec:optimizations} answers with a
$2{\times}2$ ablation.
...
documented failure mode. (\S\ref{sec:rigor} replicates the entire grid over
$K{=}15$ independent instances per cell with confidence intervals and paired
significance tests; the headline claims survive.) Baselines are \texttt{minorminer}
and
...
These are the results of the preliminary algorithm of \S\ref{sec:reweave}; the
optimized production version and its (improved, and much faster) numbers follow in
\S\ref{sec:optimizations}, so we summarize here and defer the full preliminary
tables---per-cell quality, wall-clock, and cross-topology robustness, with the
density trend---to Appendix~\ref{app:prelim}
(Tables~\ref{tab:quality}--\ref{tab:robust}, Figure~\ref{fig:density}). All four
methods embed every instance across all seeds and topologies ($100\%$ success), so
the comparison is on chain length, its variance, and time.
...
empirically. It consumes the same \texttt{minorminer} base for a given seed and
only refines it, trimming ACL on the denser cells while leaving sparse cells,
where the base is already near-optimal, unchanged. Second, the substantive gains
come from \texttt{reweave-thorough}, whose four seeded restarts both lower the
mean and, by keeping the best of several runs, sharply cut the run-to-run variance
that is \texttt{minorminer}'s signature weakness: over the complete $35$-cell grid
(Appendix~\ref{app:full}) it lowers mean ACL on \emph{every} cell (by $1$--$11\%$;
the largest single gain, $-10.6\%$, is on a $d$-regular source) and cuts ACL
standard deviation in $30$ of the $35$, by up to $76\%$ (e.g.\
$0.11\!\to\!0.03$ at clean-$P_6$ $n{=}30,d{=}0.3$); the five exceptions are cells
where \texttt{minorminer}'s spread is already small ($0.02$--$0.09$), and we
report them rather than hide them. The effect persists on broken Pegasus---where
\texttt{minorminer}'s ACL inflates most and the thorough gain is largest
($-7.3\%$ at $n{=}40,d{=}0.5$)---and on Zephyr, so the mechanism is not
topology-specific. The cost is wall-clock: the preliminary pure-Python router runs
$3$--$6\times$ compiled \texttt{minorminer}, and \texttt{reweave-thorough} can use
the full $20$\,s budget (Table~\ref{tab:timing})---the gap
\S\ref{sec:optimizations} closes; \texttt{minorminer-layout}, by contrast, is
neither consistently faster nor consistently better than plain
\texttt{minorminer}.
...
\paragraph{Which imported ingredient carries the gain?} \S\ref{sec:novelty} asked
whether it is \texttt{minorminer}'s congestion signal or its inner step that costs
the most. A $2{\times}2$ ablation on the production improver answers it: we run
Reweave with the SPH tree replaced by MM's union-of-independent-paths inner step
(\texttt{-ablate-paths}), with congestion pricing disabled so the destroy step
routes at uniform cost (\texttt{-ablate-nocong}), and with both, on the six
LNS-active dense ER cells ($K{=}5$ instances $\times\,3$ seeds, clean $P_6$; same
seeds $\Rightarrow$ same \texttt{minorminer} base, so differences isolate the
improver ingredient). The result is a finding we did not expect: \emph{either
ingredient alone is statistically interchangeable with the full algorithm}
($-1.86\%$ and $-1.89\%$ ACL vs.\ \texttt{minorminer}, against the full improver's
$-1.93\%$; paired $p=0.23$ and $0.78$), while removing \emph{both} is
significantly worse ($-1.64\%$, paired $p=0.002$)---the two ideas are individually
redundant but jointly load-bearing. Congestion pricing does earn its keep on
\emph{time}: without it, unpriced shortcuts displace more chains and the repair
work grows (${\sim}25\%$ more improver wall-clock at equal quality). The honest
attribution is therefore that most of Reweave's gain comes from the warm-started
anytime LNS \emph{structure} itself---rip up the longest chain, shortcut, repair
the displaced, keep only strict improvements---with the routing-imported
ingredients supplying a modest, mutually substitutable quality increment and the
congestion price paying for itself in speed. This mirrors the rip-up finding of
\S\ref{sec:searchguide}: on feasible instances the improver converges to much the
same local optimum along many paths; \emph{that it improves at all} is the value.
...
train/val graphs). Both curves plateau within a handful of epochs at exactly the
loss of the constant global-mean predictor---the first symptom of the posterior
collapse diagnosed in \S\ref{sec:learn-collapse} (model
...
lowers mean ACL (Table~\ref{tab:mmfork}); under the $K{=}15$ multi-instance design
of \S\ref{sec:rigor} the single fixed order leaves \emph{seed} variance essentially
unchanged ($1.02\times$ \texttt{minorminer}'s), so the variance reduction below
belongs to the portfolio, not to any one order. Order quality is vehicle- and
instance-dependent
...
ACL vs.\ \texttt{minorminer} and $-3.1\%$ vs.\ \texttt{reweave} at half
\texttt{minorminer}'s seed variance ($0.53\times$, \S\ref{sec:rigor}), the
strongest single quality result in
...
sources over a size/density/topology mix, $7$ cells, $3$ seeds; mean ACL; the
parenthesized figure is the dispersion of the seven cell means, a cross-cell
spread---per-seed variance is measured properly in \S\ref{sec:rigor}). \texttt{mmfork}
(no order) reproduces
\texttt{minorminer} exactly. A fixed Cuthill--McKee order and especially the
per-instance portfolio cut ACL. The last column is wall-clock
...
comes from seeding Reweave with \texttt{mmfork-portfolio} (\texttt{reweave+portfolio}
in Table~\ref{tab:stack}): it stacks two independent gains---a better-ordered base
\emph{and} the negotiated destroy-repair improver---but inherits the portfolio's
${\sim}6\times$ cost as its base. The practical sweet spot on \emph{dense ER}
instances is to seed from the single
Cuthill order instead: \texttt{reweave-mmfork-cuthill} (abbreviated
\texttt{reweave+cuthill} in figures) reaches ${\sim}{-}5\%$ ACL
versus \texttt{minorminer} on this grid---matching the ${\sim}6\times$ portfolio and
beating both
plain \texttt{reweave} ($-3.4\%$) and \texttt{mmfork-cuthill} ($-2.2\%$) on their
own---at only ${\sim}1.8\times$ \texttt{minorminer}, the cost of plain Reweave.
That parity is a dense-ER phenomenon, however: pooled over the full $K{=}15$
grid of \S\ref{sec:rigor} the cheap stack retains $-1.7\%$ against the portfolio's
$-5.5\%$, and on sparse $6$-regular sources at hardware scale it is statistically
tied with \texttt{minorminer} (\S\ref{sec:scaling})---seed from the portfolio when
quality is worth $6\times$, from Cuthill when the sources are dense. The
...
(\textbf{dense-ER grid}: ER into $P_6$, mean over four size/density cells
$\times3$ seeds; $t/$MM is
wall-clock relative to \texttt{minorminer}, back-to-back). On these dense cells the
cheap stack,
\textbf{\texttt{reweave-mmfork-cuthill}}, matches the ${\sim}6.5\times$ portfolio's
ACL at plain Reweave's cost; only the portfolio-seeded \texttt{reweave+portfolio}
goes lower, and it pays the portfolio's ${\sim}6\times$ as its base. Pooled over
the full $K{=}15$ grid the ordering differs (\S\ref{sec:rigor},
...
\texttt{reweave+portfolio}           & \textbf{4.801} & $-8.3\%$ & $7.4\times$ \\
\bottomrule
\end{tabular}
...
back to ${\sim}1.7\times$. The $6.4\times$ of \texttt{reweave+portfolio}
(Table~\ref{tab:fgtime}) deserves a word, because it is \emph{not}
reweave\,($1.7\times$) plus an ordered search\,($1.0\times$): it is Reweave seeded
from the
...
ACL at ${\sim}1.8\times$ on dense ER, which is the configuration to use there. Finally, reducing
...
\texttt{mm}${=}$\texttt{minorminer}, \texttt{mm-layout}${=}$\texttt{minorminer-layout},
\texttt{mf}${=}$\texttt{mmfork}, \texttt{rw+portf}${=}$\texttt{reweave+portfolio}.}
\label{tab:fgtime}
\small
\begin{tabular}{lrrrrr}
\toprule
cell & \texttt{mm} & \texttt{mm-layout} & \texttt{mf-cuth} & \texttt{mf-portf} & \texttt{rw+portf} \\
\midrule
...
order is real: a per-instance portfolio of orders lowers \texttt{minorminer}'s ACL
by ${\sim}5\%$ and halves its seed variance, a deterministic Cuthill--McKee order
captures a smaller gain for free (leaving variance unchanged), and the guidance
composes with Reweave for the best dense-ER result. Unlike a learned or
constructive embedder, none of it sacrifices \texttt{minorminer}'s reliability. The full
...
\paragraph{The relative cost of guidance stays bounded as $n$ grows.} The time curve
(Figure~\ref{fig:scaling}, averaged over $K{=}8$ instances per size) shows the guided
methods' wall-clock multipliers \emph{flat-to-falling} as the problem grows---none
compound with \texttt{minorminer}'s super-linearly-growing search. On $Z_{15}$ the
single fixed order \texttt{mmfork-cuthill} trends from $1.2\times$ \texttt{minorminer}
at $n{=}40$ to $0.9{\pm}0.1\times$ at $n{=}320$, reaching parity at scale because a
good fixed order can find a valid embedding before \texttt{minorminer}'s randomized
search does; \texttt{reweave-mmfork-cuthill} holds at ${\sim}1.2$--$1.5\times$ and the
six-search portfolio at ${\sim}5$--$7\times$ (its multiplier does not grow, though the
across-instance spread is too wide to claim it shrinks). $P_{16}$ is similar, the
single-order premium staying modest (within ${\sim}1.5\times$). (An earlier single-instance
reading showed a sharper drop---\texttt{mmfork-cuthill} apparently \emph{halving} to
$0.6\times$; instance-averaging tempers that to ``bounded, reaching parity for the
single order,'' a concrete illustration of why \S\ref{sec:rigor}'s multi-instance
design matters.) The fixed cost of computing an order, or of running several,
therefore amortizes rather than compounds: a method slower at small $n$ is no more
expensive---and for the single fixed order, eventually free---at the large $n$ that
real hardware makes reachable.
...
\addplot[blue,thick,mark=triangle*] coordinates {(40,1.19)(80,1.26)(160,1.05)(320,0.92)}; \addlegendentry{mmfork-cuthill}
\addplot[blue,thick,densely dashed,mark=triangle,forget plot] coordinates {(40,1.34)(80,1.03)(160,1.10)(320,1.14)};
\addplot[green!55!black,thick,mark=square*] coordinates {(40,1.48)(80,1.30)(160,1.39)(320,1.22)}; \addlegendentry{reweave+cuthill}
\addplot[green!55!black,thick,densely dashed,mark=square,forget plot] coordinates {(40,1.48)(80,1.17)(160,1.17)(320,1.42)};
\addplot[red,thick,mark=otimes*] coordinates {(40,6.33)(80,6.89)(160,6.95)(320,5.16)}; \addlegendentry{mmfork-portfolio}
\addplot[red,thick,densely dashed,mark=otimes,forget plot] coordinates {(40,6.87)(80,5.95)(160,6.63)(320,6.89)};
\end{axis}
\end{tikzpicture}
\caption{Wall-clock relative to \texttt{minorminer} ($=1$) versus source size $n$
($6$-regular sources, mean over $K{=}8$ instances; \textbf{solid} $=$ Advantage2
$Z_{15}$, \textbf{dashed} $=$ Advantage $P_{16}$; $5\%$ faults). The guided methods'
multipliers stay \emph{bounded}---flat-to-falling---rather than compounding with
\texttt{minorminer}'s super-linearly-growing search: the single order
\texttt{mmfork-cuthill} reaches parity ($0.9\times$) on the largest $Z_{15}$ problems,
and the six-search portfolio's ${\sim}6\times$ does not grow with $n$.}
\label{fig:scaling}
\end{figure}

\paragraph{Quality stays competitive; the frontier is shared.} On ACL, averaging
over $K{=}8$ instances sharpens the picture (Figure~\ref{fig:scalingacl}):
\texttt{mmfork-portfolio} sits a consistent $5$--$7\%$ below \texttt{minorminer} at
every size, with $95\%$ confidence intervals that clear $1$ throughout---a persistent,
not widening, advantage. The single-order \texttt{mmfork-cuthill} and
\texttt{reweave-mmfork-cuthill}, by contrast, are statistically \emph{tied} with
\texttt{minorminer} on this sparse $6$-regular regime (their intervals straddle $1$):
ordering buys less on sparse-into-huge than on the dense ER cells of
\S\ref{sec:results}, where chains run long enough for a better order to matter. The
...
hardware the order-guided variants keep \texttt{minorminer}'s reach and reliability at
bounded extra time, with \texttt{mmfork-portfolio} delivering a consistent $5$--$7\%$
ACL reduction at scale (the quality choice) and the near-free \texttt{mmfork-cuthill}
the conservative drop-in.
...
\begin{axis}[width=\linewidth, height=4.8cm, xmode=log, log basis x=2,
  ...
  ylabel={ACL $/$ \texttt{minorminer}}, ymin=0.87, ymax=1.11,
  legend pos=north west, legend columns=2,
  legend style={font=\scriptsize, fill opacity=0.85},
  grid=both, grid style={gray!18},
  error bars/error bar style={line width=0.5pt}]
\addplot[black,thick,mark=*] coordinates {(40,1)(80,1)(160,1)(320,1)}; \addlegendentry{minorminer}
\addplot[blue,thick,mark=triangle*,error bars/.cd, y dir=both, y explicit] coordinates {(40,1.000)+-(0,0.040) (80,1.034)+-(0,0.042) (160,1.038)+-(0,0.033) (320,1.023)+-(0,0.049)}; \addlegendentry{mmfork-cuthill}
\addplot[green!55!black,thick,mark=square*,error bars/.cd, y dir=both, y explicit] coordinates {(40,0.995)+-(0,0.041) (80,1.017)+-(0,0.041) (160,1.005)+-(0,0.034) (320,0.985)+-(0,0.046)}; \addlegendentry{reweave+cuthill}
\addplot[red,thick,mark=otimes*,error bars/.cd, y dir=both, y explicit] coordinates {(40,0.928)+-(0,0.035) (80,0.942)+-(0,0.018) (160,0.946)+-(0,0.033) (320,0.947)+-(0,0.023)}; \addlegendentry{mmfork-portfolio}
\end{axis}
\end{tikzpicture}
\caption{ACL relative to \texttt{minorminer} ($=1$) versus $n$ ($6$-regular into
Advantage $P_{16}$; mean over $K{=}8$ instances, $95\%$ CI bars).
\texttt{mmfork-portfolio} sits a consistent $5$--$7\%$ below \texttt{minorminer} at
every size (CI clears $1$ throughout)---the advantage \emph{persists} rather than
widening; the single-order variants are within noise of \texttt{minorminer} on this
sparse regime.}
...
\paragraph{Shorter chains give better solutions---within a fixed problem.} Across
the whole cloud of (problem, embedding) pairs the association is striking: at the
reference chain strength the pooled Spearman correlation between ACL and
$P(\mathrm{GS})$ is $\rho=-0.63$, and between ACL and chain breaks $+0.80$
(Figure~\ref{fig:quality}). But the pooled statistic conflates two effects: harder
problems (larger $n$, higher density) are simultaneously harder to solve \emph{and}
longer-chained, so much of that correlation is problem difficulty, not embedding
quality. The claim the paper actually needs is the \emph{within-problem} one, and it
holds: demeaning within each (problem, target)---so only embedding-to-embedding
differences on the \emph{same} problem remain---ACL still anti-correlates with
$P(\mathrm{GS})$ (fixed-effects $\rho=-0.11$, $p=3{\times}10^{-8}$; per-problem
median $\rho=-0.13$, negative for $62\%$ of the $196$ problem--target units,
sign-test $p=1.2{\times}10^{-4}$), and residual energy ($+0.10$,
$p=2{\times}10^{-7}$) and chain breaks ($+0.15$, $p=6{\times}10^{-16}$) move with
it; the demeaned slope is $-0.28$ in $P(\mathrm{GS})$ per unit of ACL. The
fixed-problem effect is an order of magnitude smaller than the pooled cloud
suggests---embeddings of the same small problem differ by only ${\sim}0.3$ ACL
(median within-problem range), so the lever's travel is short here---but it is
real, significant, and mechanistic: longer chains break more often under a fixed
coupler budget, and broken chains corrupt the majority vote. The link is not an
artifact of one chain-strength choice (at each embedding's \emph{best} strength the
pooled correlation is still $\rho=-0.53$), and Figure~\ref{fig:chainstrength}
exhibits the mechanism: short chains are
...
probability falls monotonically with ACL (pooled $\rho=-0.63$); (b) the
chain-break fraction rises with ACL (pooled $\rho=+0.80$)---the mechanism. The
pooled trend conflates problem difficulty with embedding quality; the
fixed-effects analysis in the text isolates the embedding-specific effect
($\rho=-0.11$, $p=3{\times}10^{-8}$).}
\label{fig:quality}
\end{figure}
...
$\ge1.78$; tertiles pool across problems, so part of the separation reflects
problem difficulty---the mechanism, not the effect size, is the point here). (a)
longer chains need a stronger ferromagnet and still cap at a lower
...
Wilcoxon test across problems ($p=2\times10^{-4}$, medium effect)---a within-problem
comparison, immune to the pooling confound above. In practical terms the portfolio's
embeddings raise mean $P(\mathrm{GS})$ from $0.594$ to $0.649$, a ${\sim}9\%$
relative improvement---about $9\%$ fewer anneal reads for the same expected number
of ground-state hits at these sizes. The per-method
\emph{gaps} are modest because the brute-force ceiling forces small sources where
all methods produce short chains (ACL ${\approx}1.5$--$1.7$).
...
\small
\centering
\input{data/tab_solquality.tex}
\end{table}

\paragraph{Does the link persist at deployment scale?} The exact-reference design
caps sources at $n\le18$, where chains stay short (ACL $\le2.8$); the paper's
methods differ most at ACL $3.5$--$5.5$. We therefore ran an SA-only extension at
$n\in\{24,32,40\}$ (ER $d{=}0.5$ into clean $P_6$; ACL $2.5$--$4.9$), scoring each
run against the best energy found across all $36$ runs of its problem, since the
exact ground state is out of reach. We report it honestly: the extension is
\emph{directionally consistent but statistically inconclusive}. The lowest-ACL
methods again hit the best-known energy most often (\texttt{mmfork-portfolio}
$0.080$ vs.\ \texttt{minorminer} $0.030$ at reference strength; per-problem median
fixed-effects $\rho=-0.16$, negative for $65\%$ of problems), but with only $24$
problems nothing reaches significance ($p\ge0.27$), and the residual-energy metric
is ambiguous at some strengths. Two effects plausibly blur the channel at this
scale beyond low power: chain breaking saturates ($21$--$42\%$ of chains break at
the swept strengths, all methods alike), and majority-vote decoding then acts as a
classical repair that masks much of the difference---a masking real hardware, with
its analog dynamic-range and noise costs, does not enjoy. Extending the controlled
result into this regime---more problems, stronger coupling, decoders weaker than
majority vote, and ultimately hardware---is the natural next experiment.
...
hundreds of physical qubits these embeddings occupy. The claim we support is
therefore the controlled one: \emph{at fixed problem and sampler, in the regime
where chain breaking is not saturated, shorter and more
uniform chains yield measurably better solutions}---so the ACL reductions of the
preceding sections are reductions in something that matters, with the effect size
at fixed problem modest for classical sampling and its full practical weight
expected from hardware channels our simulators do not model.
...
$\times\,3$ algorithm seeds $=1{,}575$ embeddings per method; $30$\,s per-trial
budget; instance seeds disjoint from the fixed-instance grid's), and analyze the
$525$ per-(cell, target, instance) units with bootstrap confidence intervals and
Holm--Bonferroni-corrected paired Wilcoxon tests, separating graph-draw variance from
algorithm-seed variance. One dependency caveat: the same ER instance appears under
all three targets ($525$ units from $255$ distinct graphs), which the tests treat as
independent; collapsing each graph's targets into a single unit---$255$ fully
independent graphs---leaves every conclusion below intact
(e.g.\ \texttt{reweave} $p=8{\times}10^{-28}$, \texttt{reweave-thorough} and
\texttt{mmfork-portfolio} $p=4{\times}10^{-43}$, \texttt{mmfork-cuthill}
$p=8{\times}10^{-4}$).
...
\texttt{reweave-stacked}, and its failure mode is a distribution, not a mean: the
\emph{typical} instance is unchanged (median paired difference $-0.01$ qubits/chain,
Wilcoxon $p=0.41$), but a heavy tail of small, sparse instances regresses badly
(worst $+72\%$), dragging the mean to $+2.6\%$ \emph{worse} than \texttt{minorminer}
with a CI that excludes zero ($[+1.7,+3.7]$)---the multilevel base inflates ACL
exactly where \texttt{minorminer} is already near-optimal, even as it helps on
large dense instances (\S\ref{sec:optimizations}). This is why we report it as an
optional variant rather than a default; the
...
\texttt{reweave-stacked}'s mean CI excludes zero while its median is null---a heavy
tail, not a shift; see text.}
\label{tab:instances}
\small
\centering
\input{data/tab_instances.tex}
\end{table}
...
the embedded sizes here remains out of reach; its controlled, exact-reference
regime is also capped at short chains, and the larger-$n$ extension is
directionally consistent but statistically inconclusive---majority-vote decoding
repairs much of what chain breaking costs a classical sampler. Direct confirmation
on quantum hardware, where no such classical repair is free, is the natural next
step.
...
\texttt{minorminer}'s variability is \emph{order}-driven, so we open its vertex
order with a small fork: a per-instance portfolio of bandwidth-reducing orders
lowers its ACL by ${\sim}5\%$ and roughly halves its seed variance (a single
Cuthill--McKee order gives a smaller gain for free),
...
this guidance stays cheap at scale: averaged over instances its wall-clock premium
does not grow with problem size, reaching parity with \texttt{minorminer} for the
single fixed order on the largest Zephyr problems. What does \emph{not} pay off is learning the placement: the naive supervised
...
\appendix
\section{Preliminary-engine tables}
\label{app:prelim}

Full results for the preliminary engine of \S\ref{sec:reweave}, summarized in
\S\ref{sec:results}: embedding quality on clean Pegasus~$P_6$
(Table~\ref{tab:quality}), the corresponding wall-clock (Table~\ref{tab:timing}),
robustness across topologies (Table~\ref{tab:robust}), and the density trend
...
\begin{tabular}{lcccc}
\toprule
cell & \texttt{mm} & \texttt{mm-layout} & \texttt{reweave} & \texttt{rw-thor} \\
\midrule
...
The Ember benchmarking framework, every embedder evaluated here (under the
registered names used in the text), the \texttt{minorminer} fork patch and its
one-command build script, all sweep and analysis scripts, and the per-run CSVs
behind every table and figure ship in the artifact accompanying this submission;
\texttt{docs/paper/reproduce.sh} regenerates the locally-reproducible pipelines
end-to-end and documents the cluster-scale sweeps. An archival public repository
link will be provided upon publication (withheld here for double-anonymous
review).
