...
mean ACL (up to ${-}11\%$) at a fraction of its former cost.
\end{abstract}
...
This paper makes four contributions. (i) We frame minor-embedding chain
...
${\sim}1.3\times$ compiled \texttt{minorminer}'s wall-clock. We are explicit
...
(no gain over the shortest-path heuristic), and left a compiled (numba) routing
core and parallelized restarts as future speed work.
...
PathFinder's routing is pure Python. The optimizations of
\S\ref{sec:optimizations} bring single-seed \texttt{pathfinder} to within
${\sim}1.3\times$ \texttt{minorminer}'s wall-clock via bounded-region routing; the
remaining gap (and the thorough configuration's restarts) would close with a
compiled (numba) Dijkstra core---which we measured bit-identical and ${\sim}2\times$
faster, omitted from the default only to avoid the dependency---and with
parallelized restarts. The standalone cold start
...
little time cost. The primitives are classical; the synthesis, application,
optimization, and empirical validation are the contribution.
