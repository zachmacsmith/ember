# PathFinder — negotiated rip-up-and-reroute minor embedding

PathFinder is a new minor-embedding algorithm for ember-qc (research-brief
...
| `pathfinder*` | `ember_qc/algorithms/pathfinder.py` | The negotiated rip-up-and-reroute embedder (3.5) |

The embedder *interface* (§2.1) already existed as `EmbeddingAlgorithm` +
`@register_algorithm`; PathFinder conforms to it, so it drops into the existing
...
a congested fabric. PathFinder ports the two routing ideas MM leaves on the
table, as the smallest useful delta from MM:

1. **A real Steiner inner step.** MM rebuilds a vertex's chain as a *union of
   independent shortest paths* to its placed neighbours. PathFinder grows one
...
2. **Negotiated congestion (McMurchie–Ebeling 1995).** MM prices an over-used
   qubit by `diam(G)^k` — a per-pass snapshot with no memory. PathFinder prices
...
~3.9–8 vs MM ~2.3). So PathFinder's headline mode **warm-starts from a fast base
...
| Name | Mode | Notes |
|------|------|-------|
| `pathfinder` | MM-seeded improver | Balanced; never worse than MM, equal-base comparison |
| `pathfinder-thorough` | best-of-4 restarts + deeper reroute | Lower ACL & variance; ~4× the time |
| `pathfinder-cold` | standalone BFS cold start | No MM seed; documents the placement gap |
...
r = benchmark_one(source, target, "pathfinder", timeout=30, seed=0)
...
```bash
ember run --graphs "1-60" --algorithms minorminer,minorminer-layout,pathfinder,pathfinder-thorough \
...
| instance (ER→P6) | `minorminer` ACL±std | `minorminer-layout` | `pathfinder` | `pathfinder-thorough` |
|---|---|---|---|---|
...
`pathfinder-thorough` (best-of-4) vs `minorminer`, per cell: **−4.5% to −5.2%
...
Wall-clock: MM 0.3–0.8 s, `pathfinder` 1.3–3.8 s, `pathfinder-thorough`
4.9–15.6 s (pure Python). `pathfinder` (single-seed) is the fair *equal-base*
point: ≤ MM ACL and ≤ MM variance in every cell, never worse.

**Takeaways**

- PathFinder is **valid on every instance** and, by construction, **never
  regresses** below its MM base.
- On `n ≥ 30` it lowers mean ACL by ~1–2% (`pathfinder`) and ~5–8%
  (`pathfinder-thorough`, best-of-4), and lowers ACL **variance** in most cells —
  the headline metric vs MM.
- The cost is wall-clock (pure Python; ~3–6× MM for `pathfinder`, more for
