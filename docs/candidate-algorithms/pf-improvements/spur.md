# PathFinder variant: `pathfinder-spur` — spur-pruning post-pass (Q3)

**Module:** `packages/ember-qc/src/ember_qc/algorithms/pf_spur.py`
**Registered name:** `pathfinder-spur`
**Baseline (frozen):** `pathfinder`
...
post-pass on PathFinder's final, already-valid embedding.
...
- `benchmark_one(K6, chimera_graph(4), 'pathfinder-spur')` → `is_valid=True`,
...
- Never prints; inherits a `version` str (`"2.0.0"`). Respects the deadline (the
  prune runs only once, after PathFinder's own time-budgeted `run`).
...
## Measurement — variant vs frozen `pathfinder`

`eval_variant.py pf_spur pathfinder-spur 60` (3 seeds/cell):

| cell | pathfinder ACL | spur ACL | ACL Δ% | std Δ | time × |
|---|---|---|---|---|---|
...
**GRID MEAN vs `pathfinder`: ACL −0.7%, std −0.005, time ×1.00.**
...
- The marginal gain over `pathfinder` (−0.7%) is smaller than over `minorminer`
  (−2.3%) because PathFinder's LNS already removes *some* spurs while shortening
...
deterministic; −0.7% grid-mean ACL on top of `pathfinder` and −2.3% over
...
- `docs/candidate-algorithms/data/pathfinder-spur_variant_{raw,summary}.csv` — eval output.
