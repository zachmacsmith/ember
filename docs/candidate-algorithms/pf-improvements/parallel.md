# S5 — Parallelise the restarts (`pathfinder-parallel`)

**Variant module:** `packages/ember-qc/src/ember_qc/algorithms/pf_parallel.py`
**Registered name:** `pathfinder-parallel`
**Baseline it optimises:** `pathfinder-thorough` (sequential best-of-4)
**Goal:** identical embedding quality, lower wall-clock.

## What thorough does, and what we change

`pathfinder-thorough` (`_params = {base_method:"minorminer", lns_rounds:80,
lns_penalty:4.0, base_fraction:0.4, n_restarts:4}`) runs **four independent
seed→improve restarts sequentially** inside `embed_pathfinder`, each on a
...
`pathfinder-parallel` keeps the algorithm **byte-for-byte identical** and changes
...
| ingredient | sequential thorough | `pathfinder-parallel` |
|---|---|---|
| per-restart router seed | `base_seed + i*1_000_003` | identical — worker *i* calls `embed_pathfinder(seed=base_seed + i*1_000_003, n_restarts=1)` (stride `i==0` inside, so router seed == that value) |
...
`embed_pathfinder(..., n_restarts=1)` and returns the standard result dict. The
...
`embed_pathfinder(n_restarts=4, **thorough_params)`** — i.e. ordinary thorough.
...
## Headline eval (`eval_variant.py pf_parallel pathfinder-parallel pathfinder-thorough 60`)
...
Fold the parallel-restart driver into `embed_pathfinder` (or a thin wrapper)
behind a flag, keeping all four ingredients:

1. **Top-level worker** `_pf_restart_worker(source, target, timeout, seed, params)`
   calling `embed_pathfinder(..., n_restarts=1)` — must be module-level for
   `spawn` picklability.
...
5. **Fallback:** on any pool failure or empty harvest, run the existing
   sequential `embed_pathfinder(n_restarts=n)` so the contract (never crash /
   `None`, no stdout) is unconditionally honoured.

Suggested shape: add `parallel: bool = False` (or `max_workers`) to
`embed_pathfinder`; when set and `n_restarts > 1`, run the loop in the pool
instead of sequentially. `pathfinder-thorough` can then flip it on for a free
