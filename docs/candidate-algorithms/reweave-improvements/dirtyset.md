# Reweave S4 — Dirty-set incremental LNS reroute

Isolated Reweave variant. Registered name **`reweave-dirtyset`**, module
`ember_qc/algorithms/pf_dirtyset.py`. Subclasses the **frozen** `ReweaveRouter`
and overrides only `_lns_improve`; wired in via `router_cls` (same pattern as
`reweave-spur`). Baseline (`reweave`, `embedding_backend.py`,
`reweave.py`) is untouched.

Evaluated with
`docs/candidate-algorithms/data/eval_variant.py pf_dirtyset reweave-dirtyset`
(full grid, 3 seeds, paired against `reweave` and `minorminer`). Raw/summary
...
`ReweaveRouter._lns_improve` is a **round-based full sweep**:
...
**Precise change to bake into `reweave.py::_lns_improve`:** replace the
