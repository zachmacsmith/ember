# reweave-stacked — Placement-stacked Reweave (Q1)

**Module:** `ember_qc/algorithms/pf_stacked.py`  **Variant:** `reweave-stacked`
**Baseline:** `reweave` (frozen; warm-starts from `minorminer`)

## What changed

Reweave is an improver that warm-starts from a base placement (`base_method`)
...
- baseline: `base_method="minorminer"` (fast, but MM's run-to-run variance flows
  straight into Reweave's spread)
- **`reweave-stacked`: `base_method="multilevel"`** — warm-start from the
  ...
  global structure is visible, so the seed handed to Reweave is both lower
...
Per-cell, `reweave-stacked` (multilevel base) vs frozen `reweave`:
...
Stacking Reweave on the `multilevel` placement keeps mean ACL (−1.1% grid mean,
...
Set Reweave's `base_method="multilevel"` (the single one-line change). Concretely,
...
i.e. identical to `reweave` except the warm-start placement is `multilevel`
...
`reweave` lower-variance outright, switch its own `_params` base_method to
`"multilevel"`.
