# Reweave improvement S1 — faster node-weighted Dijkstra core

**Module:** `ember_qc/algorithms/pf_fastdijkstra.py`
**Registry name:** `reweave-fastdijkstra`
**Baseline:** `reweave` (frozen = today's *un-bounded* `reweave-base`)
...
kernel is a **wash** (within noise of pure-Python `reweave`), and a full-graph
...
**Verdict:** ship it, gated on `numba`. It does not change *what* Reweave computes
...
per-neighbour Dijkstra is Reweave's dominant cost, gets ~5.8× faster; the
...
`FastDijkstraRouter(ReweaveRouter)` overrides exactly two things:
...
6 cells × 3 seeds, full `embed()` for `reweave` vs `reweave-fastdijkstra`:
...
## Results — `eval_variant.py pf_fastdijkstra reweave-fastdijkstra` (full grid, 3 seeds)
...
Adopt it, with `numba` made a dependency of the Reweave fast path:
...
* `docs/candidate-algorithms/data/reweave-fastdijkstra_variant_{raw,summary}.csv` — eval grid output.
