...
| 11 | v1.2 T1 chain (§4.15: T1a dev / T1b native / T1c beta+arms / T1d race) | run_t1_hyde06.sh @ ac38cd89 (dev_suite --topo Z12; t1b_native.yaml; p6_probes --topo Z12 --confirm-beta; t1c_arms.py; t1d_race9.py) | hyde06 | 48/40/48/30/outer5 | ~35 (~1-2 h wall) | RUNNING 2026-08-03 | glasgow smoke re-verified under §4.15 amendment 1 (Z6 3.60 s < 5 s) |
...
| 13 | T1c ramp re-run (§4.15 remedy) | run_t1c_ramp_rerun.sh @ sha stamped at launch | hyde06 | 48 | ~3 | QUEUED behind row 11 | 450 ramp rows were instrumentation failures (stale fork .so, masked build error — see §4.15 results note); stock/beta-dhat/arm rows stand |
