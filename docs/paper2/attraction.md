...
| axis_inner + cross_widen (s3.123 — slot-paired cross-axis widening: a first-sweep adoption's realized diff widens the same slot's second-sweep probe; per-pass sweep-direction alternation) | BUILT; REFUTED AS VALUE, CONFIRMED AS MECHANISM (both levers off) | the coupled sets are real and prolifically askable (ws 479 widened asks / 348 adoptions per seed, 60-75% acceptance everywhere) but adopted ≠ profitable: crystal +0.552/10 beyond tol (third confirmation set-scale cross-axis meddling scrambles the diagonal — tiles, xy@240, widen), ER −0.050 with fatter mx, ws/regular ~parity; en route the per-block adjacency reorder was convicted by its own smoke (crystal +0.99, quiet seed) and replaced by slot pairing. The fork it sharpens: joint set-move profit needs a slack-carrying view or acceptance discipline; `data/widen_probe.csv` |
