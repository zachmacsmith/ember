...
| align_insert (s3.101 — insertion = \|S\|=1 alignment DP) | VALIDATED near-inert; lever OFF | exact all-slots insertion replaces the O(n²) double-coverage proxy; board byte-identical on most cells (spin_glass +0.135 in-tol) — value is consolidation, not quality; `data/truth_probe.csv` |
