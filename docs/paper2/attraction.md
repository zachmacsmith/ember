...
| Measure-transport unpack (adjoint junction) | PARKED | wins where the coarse order is real (turán hits 6.00 optimum; lattices fall), loses where it is spectral noise (expanders) — as an unconditional INIT; its gather is subsumed by cluster moves as an E-gated proposal, but the lattice-block wins (grid/honeycomb) live in the init-time ordering that gating cannot reproduce post-pack |
