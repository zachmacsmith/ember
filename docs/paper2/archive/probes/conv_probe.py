"""
docs/paper2/data/conv_probe.py
==============================
s3.96 exact per-line converter v2, single flip (exact_convert):
required-hull claims (positions contested, not books-hulls) + the
classed-active-set class DP. Smoke (seed 0, tail="none"): corner
deficits 0 on EVERY cell; ER 1->0 deficits and the skip gate fires on
a sparse cell for the first time; regular 17->10 w/ premx 18->13;
grid 21->10; ws 32->30. Readouts: deficit/skip/ACL/max per cell at
tol = max(0.3, 0.05 x default); ship per Max's winners rule if the
board holds.

Run:  nohup .venv/bin/python docs/paper2/data/conv_probe.py \
        > docs/paper2/data/conv_probe.log 2>&1 &
...
OUT = os.path.join(HERE, "conv_probe.csv")
SEEDS = (0, 1, 2)
