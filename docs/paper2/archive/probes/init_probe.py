"""
docs/paper2/data/init_probe.py
===============================
s3.120 — the init round under the new carry default: spectral (the
incumbent: geometry + poison order), trivial (the label-luck arm),
landmark (double-BFS: twin-adjacency by construction, graph-metric
axes, no eigenstuff), random (the karma-free optimizer-strength
baseline; the s3.35 standard reinstated). Relabel jobs (-shuf) certify
label-independence and demonstrate trivial's crystal win is label
luck. Pre-stated bar for flipping landmark to default: turán at the
carry-triv level AND king/grid/ws/regular within tol of spectral.

Run:  nohup .venv/bin/python docs/paper2/data/init_probe.py \
        > docs/paper2/data/init_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "init_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("spectral", "trivial", "landmark", "random")
KW = {
    "spectral": {},                       # the new carry default
    "trivial": {"init_mode": "trivial"},
    "landmark": {"init_mode": "landmark"},
    "random": {"init_mode": "random"},
}
# relabel jobs (label-luck demonstration + independence certificate):
# for these (cell, arm) pairs one extra run under a seeded isomorphic
# relabeling, reported with arm suffix "-shuf"
RELABEL = {("turan_n162", "trivial"), ("turan_n162", "landmark"),
           ("king_graph_196", "landmark")}
...
def _run(job):
    ...
    shuf = arm.endswith("-shuf")
    base_arm = arm[:-5] if shuf else arm
    os.nice(10)
    ...
    if shuf:
        ...
        _r = _np.random.default_rng(777)
        _ns = list(src.nodes())
        _pm = dict(zip(_ns, _r.permutation(_ns).tolist()))
        src = _nx.relabel_nodes(src, _pm)
...
def main():
    ...
    for c, f, g in cells:
        ...
        for arm in ARMS:
            if (c, arm) in RELABEL:
                jobs += [(c, f, g, arm + "-shuf", s) for s in (0, 1, 2)]
