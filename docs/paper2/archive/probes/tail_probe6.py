"""
docs/paper2/data/tail_probe6.py
===============================
s3.84 part 2: the last cell of the 2x2. {chain, ball} x {det, rng}:
ball-det, ball-rng, and chain-det (shorten, probe5) all measured ~equal
failures vs the sandwich; minorminer occupies the untested cell —
per-CHAIN moves with RANDOMIZED interrogation. rshorten+ball = our
shortener with mm's channels (per-sweep chain-order shuffle, rng root
ties, patience 2, unbounded sweeps under the deadline), ball after, no
minorminer post-gate. Near mm+ball => mm's mechanism = granularity x
randomness JOINTLY, and it is now rebuilt natively (exorcism complete).
Near shorten+ball => something in mm remains unnamed.

Run:  nohup .venv/bin/python docs/paper2/data/tail_probe6.py \
        > docs/paper2/data/tail_probe6.log 2>&1 &
...
OUT = os.path.join(HERE, "tail_probe6.csv")
SEEDS = (0, 1, 2)
TURAN_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("mm+ball", "rshorten+ball")
...
def main():
    ...
    print("\nsummary (mean ACL; d = rshorten+ball - mm+ball, negative = wins):")
