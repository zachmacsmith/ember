"""
docs/paper2/data/default_probe.py
==================================
s3.117 — the default flip (Max: "we can go ahead and make this version
the default") + projection centering attribution. Arms: lex (the old
default), plane-nocenter (the new default with the centering
translation patched to (0,0) — probe-local monkeypatch, no production
knob), plane (as shipped: brick projection + s-aligned centering).
Bars: no regression beyond tol vs lex anywhere the s3.116 board did
not already accept; centering must not lose anywhere (stair-neutral by
construction — a loss is claim-layer information).

Run:  nohup .venv/bin/python docs/paper2/data/default_probe.py \
        > docs/paper2/data/default_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "default_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("lex", "plane-nocenter", "plane")
KW = {
    "lex": {"engine": "lex"},
    "plane-nocenter": {},   # default engine, centering patched out
    "plane": {},            # THE new default, as shipped
}
...
def _run(job):
    ...
    from ember_qc.algorithms.factored import field as _field
    _orig_center = _field._center_shift
    if arm == "plane-nocenter":
        _field._center_shift = lambda *_a, **_k: (0, 0)
    t0 = time.perf_counter()
    try:
        r = attract_embed(src, target, timeout=TIMEOUT, seed=seed, **kw)
    finally:
        _field._center_shift = _orig_center
