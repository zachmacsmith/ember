"""
docs/paper2/data/orient_probe.py
================================
s3.99 orientation flips (the y-rule relaxation): per-edge orientation
bits initialized from the diagonal rule, improved by strict-descent
flips inside the readout. Hypothesis on record: the extreme-rank hub
pathology (one arm paying a whole neighbourhood hull) drives liquid
blight, so the flip arm should improve max_chain on ws/regular with
final ACL at worst parity; dense diagonal crystals are mirror-symmetric
per edge, so turan should be a near-no-op. Decision rule (house
convention): wins beyond tol = max(0.3, 0.05 x default) on any cell
with no regression beyond tol elsewhere and no wall blowup -> default
flip is Max's call on the numbers.

Run:  nohup .venv/bin/python docs/paper2/data/orient_probe.py \
        > docs/paper2/data/orient_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "orient_probe.csv")
SEEDS = (0, 1, 2)
...
def _run(job):
    ...
    return dict(cell=cell, fabric=fabric, gid=gid, arm=arm, seed=seed,
                ...
                flip_acc=d.get("flip_accepts"),
                flip_sweeps=d.get("flip_sweeps"),
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=24) as ex:
        for row in ex.map(_run, jobs):
            print(f"{row['fabric']} {row['cell']:<18} {row['arm']:<8} "
                  ...
                  f"mx={row['max_chain']} flips={row['flip_acc']} "
    ...
    for cell, fabric, _ in cells:
        ...
        for arm in ARMS[1:]:
            ...
            line += (f" | {arm}={o[0]}({o[1]}) mx={o[2]} flips={o[4]}"
