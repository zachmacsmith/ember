"""
docs/paper2/data/fold2_probe.py
===============================
s3.98 consolidation gate: does fold_moves still earn anything on the
current baseline (unbounded packer + exact converter defaults)? Its
pre-s3.93 win (ws -0.184 both fabrics) targeted a residual the
infinite packer has since dissolved (ws 2.55 without folds). Decision
rule (pre-registered, Max): fold wins any cell beyond tol = max(0.3,
0.05 x default) with no regression beyond tol elsewhere -> PARK;
otherwise -> DELETE (archive 09467299 holds it).

Run:  nohup .venv/bin/python docs/paper2/data/fold2_probe.py \
        > docs/paper2/data/fold2_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "fold2_probe.csv")
SEEDS = (0, 1, 2)
