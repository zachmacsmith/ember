"""
docs/paper2/data/tail_probe4.py
===============================
s3.83: ball v3 — one question per chain from its obligation hull,
regenerated every pass, no constants (inflation/cap/floor/windows/units
all deleted). Arms re-test the ladder: mm+ball (the shipped sandwich),
ball (pure, minorminer-free after the gate), ball+mm (large->small
ladder, the s3.80 failure re-tested now that fat-chain repair questions
exist every pass). Under v3 code all three arms use the NEW question set
automatically. Verdicts: (a) ladder — ball+mm no longer regresses
turan/ws; (b) exorcism — ball parity with mm+ball on gate-fired cells;
(c) clique cost — K100/K140 ball wall small, ~0 accepts.

Run:  nohup .venv/bin/python docs/paper2/data/tail_probe4.py \
        > docs/paper2/data/tail_probe4.log 2>&1 &
...
OUT = os.path.join(HERE, "tail_probe4.csv")
SEEDS = (0, 1, 2)
TURAN_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("mm+ball", "ball", "ball+mm")
...
def main():
    ...
    print("\nsummary (mean ACL; d = ball+mm - mm+ball, negative = wins):")
