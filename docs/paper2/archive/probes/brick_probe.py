"""
docs/paper2/data/brick_probe.py
================================
s3.109 the brick plane: the seat engine with the along-axis ruler
quantized to the fabric parity period (brick_plane=True) vs the stock
junction ruler, both under arrange_mode="seats" — ONE flip. Hypotheses
on record (notes s3.107/s3.108b): whole-brick booking removes the
phantom half-qubit savings and the parity-classed converter-miss class
(smoke: turan cmiss 43 -> 24-35, certified at 240s); the open question
is board-wide quality within the standard budget. P16 pairs are a
gate check: brick is stride-gated, the arms must tie there.

Run:  nohup .venv/bin/python docs/paper2/data/brick_probe.py \
        > docs/paper2/data/brick_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "brick_probe.csv")
SEEDS = (0, 1, 2)
...
KW = {
    ...
    "brick": {"arrange_mode": "seats", "brick_plane": True},
}
