"""
docs/paper2/data/lex_probe.py
==============================
s3.110 the two-ruler engine: arrange_mode="lex" (lexicographic
(capacity, stair) descent — brick-ruler constraint, junction-ruler
objective, no pack moves, no lam; one packer pass at each end: init
projection + the family normalizer the converter stack requires,
measured 578 -> 0 deficits with pen 0 preserved) vs the seats control
AND the shipped orders default (the beating-minorminer bar). diag
seat_pen == 0 rows certify the feasibility invariant held.

Run:  nohup .venv/bin/python docs/paper2/data/lex_probe.py \
        > docs/paper2/data/lex_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "lex_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "seats", "lex")
KW = {
    "default": {},
