...
=================================
s3.95: the s3.86 autopsy RE-RUN on the s3.93 unbounded baseline —
does the grind still EDIT our patterns (relocations/cascades) or
has it been demoted to coupler-snapping (in-place shrinks)? Old
baseline: cascade 1.00, half the chains displaced >3 tiles.
Original header: characterize WHAT minorminer's warm grind changes on our raw
...
Run:  nohup .venv/bin/python docs/paper2/data/grind_autopsy.py \
        > docs/paper2/data/grind_autopsy2.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "grind_autopsy2.csv")
