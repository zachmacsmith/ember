"""
docs/paper2/data/tail_probe7.py
===============================
s3.85 the lane audit: member placement by measured cost over candidate
lanes — minorminer's audition transplanted to the bar family, O(lines)
arithmetic instead of graph search. All arms share the audited rebuild;
the arms differ only in the tail composition. Anchors from probe4/6:
the plateau — turán 6.63-6.65 / ws 4.1-4.6 across every prior tail arm
vs the sandwich's 6.02 / 3.0. Verdicts sought: (1) is the plateau
broken — does the audited ball close toward the sandwich? (2) does the
sandwich itself (mm+ball) improve, since it shares the rebuild?
(3) rng-over-exact-selection — Max's order hunch: does re-rolled tie
randomness beat the deterministic exact order even when selection is
by measured cost?

Run:  nohup .venv/bin/python docs/paper2/data/tail_probe7.py \
        > docs/paper2/data/tail_probe7.log 2>&1 &
...
OUT = os.path.join(HERE, "tail_probe7.csv")
SEEDS = (0, 1, 2)
TURAN_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("mm+ball", "ball", "ball-rng")
KW = {"mm+ball": {"tail": "mm+ball"},
      "ball": {"tail": "ball"},
      "ball-rng": {"tail": "ball-rng"}}
...
def main():
    ...
    print("\nsummary (mean ACL; d = ball - mm+ball, negative = wins):")
