"""
docs/paper2/data/tail_probe5.py
===============================
s3.84: hypothesis (b)'s discriminator — is the sandwich's middle rung
just per-chain SLACK REMOVAL? shorten+ball = our native deterministic
shortener (polish.shorten_chains, never before a tail arm) where the
grind would run, ball after, no minorminer post-gate. Near mm+ball =>
minorminer exits the pipeline (the slack story holds; ladder becomes
cluster -> shorten -> ball, all ours). Near pure-ball => the grind's
edge really is stochastic wandering, slack eliminated as explanation.

Run:  nohup .venv/bin/python docs/paper2/data/tail_probe5.py \
        > docs/paper2/data/tail_probe5.log 2>&1 &
...
OUT = os.path.join(HERE, "tail_probe5.csv")
SEEDS = (0, 1, 2)
TURAN_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("mm+ball", "shorten+ball")
...
def main():
    ...
    print("\nsummary (mean ACL; d = shorten+ball - mm+ball, negative = wins):")
