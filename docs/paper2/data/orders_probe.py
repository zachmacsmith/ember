"""
docs/paper2/data/orders_probe.py
=================================
Round 1 of the orders-state design (ideas.md fronts 1+2, discussed with
Max 2026-08-26): state = the two axis orders, positions derived by the
pack readout after every move (pack_project minus monotonize), units =
contiguous intervals of the current order (no hierarchy), and the
acceptance fork — "orders" adopts every DP proposal (projected
block-coordinate descent; lexicographic E survives only in the
best-state bookmark), "orders-audit" is the strict-descent control.
Headlines: turán deep seeds (does accept-all wander off the crystal),
ws deep seeds (does the free projection fold — watch max_edge_span and
max chain), and the wall bar (one readout per adopted move).
P16 cells dropped per the s3.112 write-off.

Run:  nohup .venv/bin/python docs/paper2/data/orders_probe.py \
        > docs/paper2/data/orders_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "orders_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "orders", "orders-audit")
KW = {
    "default": {},
