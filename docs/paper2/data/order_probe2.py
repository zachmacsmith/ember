"""
docs/paper2/data/order_probe2.py
===============================
v4 stage O1 rerun (2026-08-08): the order-state migration with the
occupancy-footprint census fix (order-mode books width-floor sub-tile
arms; the first order_probe run collapsed on P16 because point arms
were invisible to line_depth — E=0.0, turan 7.9->13.1). Same board,
one flip. Under
...
Run:  nohup .venv/bin/python docs/paper2/data/order_probe2.py \
        > docs/paper2/data/order_probe2.log 2>&1 &
...
OUT = os.path.join(HERE, "order_probe2.csv")
SEEDS = (0, 1, 2)
...
def _run(job):
    ...
    kw = {"order_state": True} if arm == "order" else {}
...
def main():
    ...
    print("\nsummary (mean ACL; d = order - default, negative = wins):")
