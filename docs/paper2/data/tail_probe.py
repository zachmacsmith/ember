"""
docs/paper2/data/tail_probe.py
==============================
s3.80: the pipeline tail at equal TOTAL budget. Arms: mm (control —
warm grind only), ball+mm (ball_polish to its fixpoint, then the grind
with the remaining wall — Max's move-scale ladder), ball (no minorminer
after the ball; with the mm-skip gate fired this is a minorminer-free
Zephyr pipeline). Watch: (a) Max's bet — ball+mm >= mm everywhere;
(b) the replacement question — ball-only parity on Z12 gate-fired
cells; (c) P16 is a watch, not a bar (Zephyr-first directive).

Run:  nohup .venv/bin/python docs/paper2/data/tail_probe.py \
        > docs/paper2/data/tail_probe.log 2>&1 &
...
OUT = os.path.join(HERE, "tail_probe.csv")
SEEDS = (0, 1, 2)
TURAN_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("mm", "ball+mm", "ball")
...
def main():
    ...
    print("\nsummary (mean ACL; d = ball+mm - mm, negative = wins):")
    for cell, fabric, _ in cells:
        ...
        dg = m["ball"]
        print(f"{fabric} {cell:<18} mm={de[0]}({de[1]}) "
              f"ball+mm={o[0]}({o[1]}) ball={dg[0]}({dg[1]}) "
