"""
docs/paper2/data/strain_probe.py
================================
s3.89 the fold as a move + strain scheduling, measured as cumulative
flips against the shipped default:

  orient : gather_orient — every gather also offers its REVERSED block
           (the derived within-block order is a candidate, not a
           commitment; reversal is the fold's atom).
  fold   : + fold_moves — the flagged-edge nominator (span x merge-round,
           graph x layout) proposes two-axis hairpin composites (riffle
           on the strand axis + strand separation on the other), one
           executed composite per pass, judged by the ordinary gate with
           the ratchet relaxed on still-infeasible states.
  strain : + strain_rank — cluster composites execute in descending
           proxy-gain order instead of coarsest-first.

Pre-registered bars (outcome units): ws_n486/Z12 mean ACL and mean max
chain improve vs default at 10 seeds; no board cell regresses beyond
tol = max(0.3, 0.05 x default); mean wall within +10% per cell.
In-session smoke (seed 0, ws/Z12): default 2.685/mx10/span435 vs full
stack 2.621/mx9/span36; turan 6.00/6 both arms; K100 7.84 -> 7.80.

Run:  nohup .venv/bin/python docs/paper2/data/strain_probe.py \
        > docs/paper2/data/strain_probe.log 2>&1 &
...
OUT = os.path.join(HERE, "strain_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "orient", "fold", "strain")
KW = {
    "default": {},
    "orient": {"gather_orient": True},
    "fold": {"gather_orient": True, "fold_moves": True},
    "strain": {"gather_orient": True, "fold_moves": True,
               "strain_rank": True},
}
