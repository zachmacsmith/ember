"""
docs/paper2/data/align_probe.py
================================
s3.100 alignment reinsertion: the cluster pass's executor becomes the
interleaving DP (exact optimum over all merges of a unit with the
rest, both orientations; induced-rule pricing on y, frozen-net on x)
in place of the gather's one-position screen. Same nominations, same
gate. Hypotheses on record: ws and king improve (riffled placements —
the fold's atom — re-enter GATED); turan holds exact 6.000 10/10; no
dense regression (every accept passes the packed gate + overload veto,
the structural contrast with the s3.99 orient_flips defect); wall must
not grow (the DP costs less than one O(n^2) proxy call). Decision rule
(house convention): wins beyond tol = max(0.3, 0.05 x default) with no
regression beyond tol elsewhere and no wall blowup -> default flip is
Max's call on the numbers.

Run:  nohup .venv/bin/python docs/paper2/data/align_probe.py \
        > docs/paper2/data/align_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "align_probe.csv")
SEEDS = (0, 1, 2)
