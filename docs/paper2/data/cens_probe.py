"""
docs/paper2/data/cens_probe.py
==============================
s3.97 required-hull census, single flip (census_required): the gate
census prices the converter's actual claim spans (parity targets +
corner) instead of books hulls — the s3.73 blind spot made visible.
Verified live: 41 vs 6 census units on the ws end-state, threading
confirmed; seed-0 embeddings identical (delta too small vs stair-E to
flip decisions there) — the board decides whether it ever steers.
Certificate (`certified` diag) rides both arms as an observable;
certified-and-invalid must be 0 everywhere.

Run:  nohup .venv/bin/python docs/paper2/data/cens_probe.py \
        > docs/paper2/data/cens_probe.log 2>&1 &
...
OUT = os.path.join(HERE, "cens_probe.csv")
SEEDS = (0, 1, 2)
    ...
    with ProcessPoolExecutor(max_workers=24) as ex:
        for row in ex.map(_run, jobs):
            print(f"{row['fabric']} {row['cell']:<18} {row['arm']:<8} "
                  ...
                  f"mx={row['max_chain']} defic={row['deficit']} "
