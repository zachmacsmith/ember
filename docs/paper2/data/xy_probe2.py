"""
docs/paper2/data/xy_probe2.py
==============================
s3.121 follow-up — the MECHANISM probe at TIMEOUT=240: the 60s board
never reached the xy sweep on the mover cells (ws/regular xy_acc=0 —
the ladder consumes the budget on n>=316 under load; the 240s ws diag
showed 425 xy accepts once reached). Same pairing, deciders + ER
(does the +0.307 churn persist with slack?), arms (plane, xy).
...
Run:  nohup .venv/bin/python docs/paper2/data/xy_probe2.py \
        > docs/paper2/data/xy_probe2.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "xy_probe2.csv")
SEEDS = (0, 1, 2)
