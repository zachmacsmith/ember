"""
docs/paper2/data/seat_probe.py
===============================
s3.102 the seat engine (v5 prototype): crossfinder's loop on the ideal
plane — state = carried seats, moves = exhaustive exact single-variable
re-seat + rigid unit translation, strict descent on one objective,
proposer == judge (reference-scored acceptance). Init + adapter + tail
shared verbatim with the default. Hypotheses on record: dense parity
(init + one exact pack carry the crystal; sweeps near-no-op there),
lattices at least parity, liquids are the bet (s3.87: mm's fold = a
sequence of single-variable strictly-improving relocations — now the
native move); accept_traj is the plateau diagnostic, read either way.

Run:  nohup .venv/bin/python docs/paper2/data/seat_probe.py \
        > docs/paper2/data/seat_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "seat_probe.csv")
SEEDS = (0, 1, 2)
...
def _run(job):
    ...
    return dict(cell=cell, fabric=fabric, gid=gid, arm=arm, seed=seed,
                ...
                seat_acc=d.get("seat_accepts"),
                trans_acc=d.get("trans_accepts"),
                passes=d.get("seat_passes"),
                fmiss=d.get("seat_fast_miss"),
