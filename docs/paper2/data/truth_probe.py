"""
docs/paper2/data/truth_probe.py
================================
s3.101 truth round, cumulative arms: ins = |S|=1 alignment insertion
(replaces the O(n^2) double-coverage proxy); insreq = + the restored
s3.97 required-hull census in the gate (stride-gated); inscap = + per-
line capacity pressure in the proposal DP; reqL4 = census escalation
(overload_lam=4) in case the required term is inert at lam=1 again.
Hypotheses on record: ins wins-or-ties broadly (exact >= proxy at the
same gate); req moves ER/regular on Z12 (P16 untouched by the stride
gate); cap cuts the revert_ov share and is the arm to watch on ws;
turan stays exact 6.000 10/10 throughout; no dense regressions (all
terms evaluation-only; feasible crystals score zero on both new
terms). Decision rule (house convention): wins beyond tol =
max(0.3, 0.05 x default) with no regression beyond tol and no wall
blowup -> default flips are the owner's call on the numbers.

Run:  nohup .venv/bin/python docs/paper2/data/truth_probe.py \
        > docs/paper2/data/truth_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "truth_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "ins", "insreq", "inscap", "reqL4")
KW = {
    "default": {},
    "ins": {"align_insert": True},
    "insreq": {"align_insert": True, "census_required": True},
    "inscap": {"align_insert": True, "census_required": True,
               "cap_pressure": True},
    "reqL4": {"align_insert": True, "census_required": True,
              "overload_lam": 4.0},
}
...
def _run(job):
    ...
    return dict(cell=cell, fabric=fabric, gid=gid, arm=arm, seed=seed,
                ...
                align_p=d.get("align_props"),
                align_n=d.get("align_noops"),
                rev_ov=d.get("revert_ov"),
                rev_e=d.get("revert_e"),
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=24) as ex:
        for row in ex.map(_run, jobs):
            print(f"{row['fabric']} {row['cell']:<18} {row['arm']:<8} "
                  ...
                  f"mx={row['max_chain']} align={row['align_p']} "
    ...
    for cell, fabric, _ in cells:
        ...
        for arm in ARMS[1:]:
            ...
            line += (f" | {arm}={o[0]}({o[1]}) mx={o[2]} align={o[4]}"
