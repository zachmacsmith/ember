"""
docs/paper2/data/int_probe.py
==============================
s3.111 best_interleave (the insertion DP as a one-court move): the two
one-flip pairs (seats vs seats+interleave, lex vs lex+interleave) plus
the shipped orders default as the bar. Headline: does the jump move
rescue lex's turán (+1.326 at s3.110b, the path-blocking refutation;
smoke seed 0: 6.000 exact), does the full interleaving family beat the
6-candidate gather, and the decline-rate diagnostic (capacity-blind DP
interior vs the audit).

Run:  nohup .venv/bin/python docs/paper2/data/int_probe.py \
        > docs/paper2/data/int_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "int_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "seats", "seatsint", "lex", "lexint")
KW = {
    "default": {},
    "seats": {"arrange_mode": "seats"},
    "seatsint": {"arrange_mode": "seats", "interleave_moves": True},
    "lex": {"arrange_mode": "lex"},
    "lexint": {"arrange_mode": "lex", "interleave_moves": True},
}
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=24) as ex:
        for row in ex.map(_run, jobs):
            print(f"{row['fabric']} {row['cell']:<18} {row['arm']:<8} "
                  ...
                  f"mx={row['max_chain']} cmiss={row['cmiss']} "
    ...
    for cell, fabric, _ in cells:
        ...
        for arm in ARMS[1:]:
            ...
            line += (f" | {arm}={o[0]}({o[1]}) mx={o[2]} cmiss={o[4]}"
