"""
docs/paper2/data/carry_probe.py
================================
s3.118 — carry the order (the id-fossil dies): the engine state is the
two axis orders literally, tie-break = carried rank, every interleaver
candidate a real state, edge-pair units subsume monotonize. Arms:
plane (shipped default), carry (accept-all + carry), carry-audit.
Watch: churn collapse (adopt counts), ER (the tie-fiction confound),
turán exactness, ws max_edge_span, grid/king.

Run:  nohup .venv/bin/python docs/paper2/data/carry_probe.py \
        > docs/paper2/data/carry_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "carry_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("plane", "carry", "carry-triv", "carry-audit")
KW = {
    "plane": {},                                  # the shipped default
    "carry": {"carry_orders": True},              # s3.118, accept-all
    "carry-triv": {"carry_orders": True, "init_mode": "trivial"},
    "carry-audit": {"engine": "plane-audit", "carry_orders": True},
}
