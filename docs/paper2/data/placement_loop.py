...
def run_loop(S, T_edges, src_adj, pos, tree, qubits, degree_order, adj, seed, clock):
    """R rounds of relax->snap->route->prune; returns (best_emb, trajectory, fails)."""
    traj, fails = [], 0
...
def main(smoke=False):
    ...
    if smoke:
        ...
        print(f"smoke OK: traj={['%.2f' % t if t else 'F' for t in traj]} "
    ...
    for n in NS:
        ...
        for seed in SEEDS:
            ...
            clock = MMClock(); t0 = time.perf_counter()
            best, traj, fails = run_loop(S, T_edges, src_adj, pos, tree, qubits,
                                         degree_order, adj, seed, clock)
            pol = polish(S, T_edges, best, seed, clock) if best else None
            rows.append(dict(n=n, seed=seed, arm="loop",
    ...
    for n in NS:
        print(f"n={n}")
        for arm in ("mm-full", "loop", "control"):
        ...
        trajs = [[float(x) if x != "F" else None for x in r["trajectory"].split(";")]
                 for r in rows if r["n"] == n and r["arm"] == "loop" and r["trajectory"]]
        if trajs:
