"""
docs/paper2/data/unb_probe.py
=============================
s3.93 the infinite Zephyr packer + seed submission, 2x2 factorial:
  subm : submit_seeds — completed seeds go warm to the fallback mm
         instead of single-qubit hints (the s3.92 discard repair)
  unb  : unbounded_pack — the arrange DP packs the ideal crossbar
         (uniform lanes, unbounded lines, hard capacity unchanged, no
         boundary zeroing); the census alone prices the finite fabric;
         one bounded projection at the end
  both : the stack

Pre-registered: liquids are the target (smoke seed 0: ws unb
2.473-2.595/mx 8-9 vs default 3.088/12 — sub-minorminer for the first
time); dense must hold (smoke: K100 identical 7.84, turán 6.000 after
the line-1 anchor fix); final_width <= 25 and projection_misses ~ 0
are the convergence observables; tol = max(0.3, 0.05 x default).

Run:  nohup .venv/bin/python docs/paper2/data/unb_probe.py \
        > docs/paper2/data/unb_probe.log 2>&1 &
...
OUT = os.path.join(HERE, "unb_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("default", "subm", "unb", "both")
KW = {
    "default": {},
    "subm": {"submit_seeds": True},
    "unb": {"unbounded_pack": True},
    "both": {"unbounded_pack": True, "submit_seeds": True},
}
...
def _run(job):
    ...
    return dict(cell=cell, fabric=fabric, gid=gid, arm=arm, seed=seed,
                ...
                width_x=d.get("final_width_x"),
                width_y=d.get("final_width_y"),
                pmiss=d.get("projection_misses"),
                subm=d.get("seeds_submitted"),
    ...
    with ProcessPoolExecutor(max_workers=24) as ex:
        for row in ex.map(_run, jobs):
            print(f"{row['fabric']} {row['cell']:<18} {row['arm']:<8} "
                  ...
                  f"mx={row['max_chain']} w={row['width_x']}x"
                  f"{row['width_y']} pmiss={row['pmiss']} "
                  f"subm={row['subm']} ({row['time']}s)", flush=True)
            rows.append(row)
