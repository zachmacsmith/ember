"""
docs/paper2/data/plane_probe.py
================================
Round 3 (s3.116, Max's design): the Infinite Plane — search on the
ideal plane (readout = unbounded packs only, judge = pure stair,
capacity an invariant) with ONE brick-aware projection at the end
(per-(line, brick) pools as feasibility data; pen becomes a
true-infeasibility alarm). Arms: orders (round-2 control), plane
(accept-all, Max's candidate), plane-audit (acceptance control),
plane-h (+ hierarchy units). Watch: ws deep (does the liquid finally
move — THE prediction), turán deep (abutment conservatism + one-shot
projection), king/grid (the boundary-pen class should heal), proj_pen.

Run:  nohup .venv/bin/python docs/paper2/data/plane_probe.py \
        > docs/paper2/data/plane_probe.log 2>&1 &
Sentinel: done-probe.
"""
import csv
import os
...
OUT = os.path.join(HERE, "plane_probe.csv")
SEEDS = (0, 1, 2)
DEEP_SEEDS = tuple(range(10))
TIMEOUT = 60
ARMS = ("orders", "plane", "plane-audit", "plane-h")
KW = {
    "orders": {"engine": "orders"},
    "plane": {"engine": "plane"},
    "plane-audit": {"engine": "plane-audit"},
    "plane-h": {"engine": "plane", "hier_units": True},
}
