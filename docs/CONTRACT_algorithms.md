...
This document is the complete specification for implementing an algorithm wrapper in
EMBER. Read the strict requirements. Everything else is optional.
...
- A `SOURCE.md` documenting the source repo URL, exact commit hash, and any patches
  applied
...
```python
import pytest, hashlib, networkx as nx
...
@pytest.mark.parametrize("name", list(ALGORITHM_REGISTRY.keys()))
class TestAlgorithmContract:

    def test_returns_result_on_success(self, name, source, target):
        result = benchmark_one(source, target, name, timeout=30)
        assert result is not None
        assert isinstance(result.wall_time, float)
        assert result.cpu_time >= 0

    def test_returns_result_on_failure(self, name, impossible, target):
        result = benchmark_one(impossible, target, name, timeout=5)
        assert result is not None
        assert result.success == False
        assert result.status in {'TIMEOUT', 'CRASH', 'FAILURE', 'OOM', 'INVALID_OUTPUT'}

    def test_respects_timeout(self, name, impossible, target):
        result = benchmark_one(impossible, target, name, timeout=2)
        assert result.wall_time < 5.0, f"{name} exceeded timeout by more than 3s"

    def test_seed_reproducibility(self, name, source, target):
        r1 = benchmark_one(source, target, name, seed=42, timeout=30)
        r2 = benchmark_one(source, target, name, seed=42, timeout=30)
        if r1.success and r2.success:
            assert r1.embedding == r2.embedding

    def test_does_not_modify_inputs(self, name, source, target):
        sh = hashlib.sha256(str(sorted(source.edges())).encode()).hexdigest()
        th = hashlib.sha256(str(sorted(target.edges())).encode()).hexdigest()
        benchmark_one(source, target, name, timeout=10)
        assert hashlib.sha256(str(sorted(source.edges())).encode()).hexdigest() == sh
        assert hashlib.sha256(str(sorted(target.edges())).encode()).hexdigest() == th

    def test_counters_are_valid_types(self, name, source, target):
        result = benchmark_one(source, target, name, timeout=30)
    ...
    def test_counters_reproducible_with_seed(self, name, source, target):
        r1 = benchmark_one(source, target, name, seed=42, timeout=30)
        r2 = benchmark_one(source, target, name, seed=42, timeout=30)
        if r1.success and r2.success:
            for c in ['target_node_visits', 'cost_function_evaluations',
                      'embedding_state_mutations', 'overlap_qubit_iterations']:
                v1, v2 = getattr(r1, c, None), getattr(r2, c, None)
