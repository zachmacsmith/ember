...
GENERATED_DIR = Path("/Users/zachmacaskill-smith/Dropbox/Zach_Dropbox/Code/Research/ember/scripts/generated_graphs/New_Generation/libary/library2")


# =============================================================================
# ID ranges
# Starting ID for each graph type. IDs count up sequentially from these values.
# These are non-overlapping: each range is large enough to hold all currently
# generated instances with at least 100 IDs of headroom before the next type.
# Updated 2026-04-06 after library-wide ID reassignment to eliminate duplicates.
# Reserved: 0–999 (future QUBO).
# =============================================================================

ID_RANGES: dict[str, int] = {
    # Deterministic structured (1000–5549)
    "complete":              1000,
    "bipartite":             1200,
    "grid":                  1550,
    "cycle":                 1800,
    ...
    # Random (5550–30099)
    "random_er":             5550,
    "barabasi_albert":       8950,
    "regular":               12750,
    "watts_strogatz":        16600,
    "sbm":                   30100,
    "lfr_benchmark":         31350,
    "random_planar":         31500,
    # Physics lattices (31800–34149)
    "triangular_lattice":    31800,
    "kagome":                32100,
    "honeycomb":             32350,
    "king_graph":            32600,
    "frustrated_square":     32800,
    "shastry_sutherland":    33000,
    "cubic_lattice":         33200,
    "bcc_lattice":           33400,
    # Application (33550–37599)
    "weak_strong_cluster":   33550,
    "planted_solution":      34150,
    "spin_glass":            36900,
    # Structured special (37600–37999)
    "hardware_native":       37600,
    "named_special":         37750,
    "sudoku":                37900,
    # NP problems — reserved, not yet implemented (38000+)
    "np_problem":            38000,
}
...
def gen_lfr_benchmark(n: int, tau1: float, tau2: float, mu: float,
    ...
    Retries with progressively larger min_community if the community assignment
    fails to converge (nx.ExceededMaxIterations). Raises after 5 attempts.
    """
    base_min = max(3, min(20, n // 10))
    last_exc = None
    for attempt in range(5):
        min_community = base_min + attempt * max(1, n // 20)
        try:
            G = nx.LFR_benchmark_graph(
                n, tau1, tau2, mu,
                average_degree=average_degree,
                min_community=min_community,
                max_community=max(n // 3, min_community + 1),
                seed=seed,
            )
        ...
        return G, f"lfr_n{n}_t1{tau1:.1f}_t2{tau2:.1f}_mu{mu:.2f}_k{average_degree}_s{seed}", {
            ...
            "min_community_used": min_community,
        }
    raise nx.ExceededMaxIterations(
        f"LFR n={n} mu={mu} failed after 5 attempts (last min_community={min_community})"
    ) from last_exc
...
def _translate_row(csv_type: str, raw_params: dict[str, str], seed: int
    ...
    # ------------------------------------------------------------------
    # weak_strong_cluster: inter_edges_per_cluster not emitted by CSV generator
    # — default to 1 (one inter-cluster edge per cluster, as in original spec).
    # ------------------------------------------------------------------
    if csv_type == "weak_strong_cluster":
        return "weak_strong_cluster", {
            "n_clusters":            p["n_clusters"],
            "cluster_size":          p["cluster_size"],
            "inter_edges_per_cluster": int(p.get("inter_edges_per_cluster", 1)),
            "seed":                  seed,
        }
...
def generate_from_csv(
    ...
    *,
    ...
    Args:
        ...
        skip_existing:     If True, skip any graph whose output file already exists.
                           Safe to use when resuming an interrupted run — IDs are
                           deterministic so the same graph always maps to the same file.
    ...
    # Apply filters up front so the count is accurate
    if types_filter:
        types_set = set(types_filter)
        rows = [r for r in rows if r["graph_type"].strip() in types_set]
    if topologies_filter:
        filter_set = set(topologies_filter)
        rows = [
            r for r in rows
            if filter_set.intersection(t.strip() for t in r["topologies"].split("|"))
        ]

    active_filters = []
    if types_filter:
        active_filters.append(f"types={', '.join(sorted(types_filter))}")
    if topologies_filter:
        active_filters.append(f"topologies={', '.join(sorted(topologies_filter))}")
    filter_str = f" ({'; '.join(active_filters)})" if active_filters else ""
    print(f"Generating {len(rows)} graphs from {csv_path.name}{filter_str}...\n")
    ...
    for line_num, row in enumerate(rows, start=2):  # start=2: row 1 is header
        ...
        try:
            ...
            if skip_existing:
                expected_path = GENERATED_DIR / registry_type / f"{graph_id}_*.json"
                import glob
                if glob.glob(str(expected_path)):
                    print(f"  [skip] {registry_type}/{graph_id}_*.json already exists")
                    continue
            G, name, metadata = gen_fn(**kwargs)
            if extra_metadata:
    ...
    # Write ID_RANGES and per-type next-available IDs next to the script
    # (not inside GENERATED_DIR, so it doesn't get uploaded to HuggingFace).
    ranges_path = Path(__file__).parent / "id_ranges.json"
...
def _build_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="When using --csv, skip any graph whose output file already exists.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="When using --csv, only generate rows of these graph types "
             "(e.g. --types triangular_lattice kagome honeycomb).",
    )
