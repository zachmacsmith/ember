"""Thin generation adapter for the declared Transfer004 development inputs."""
...
OLD_PUBLIC = ROOT/'results/codex/transfer-cycle-003'
OLD_PRIVATE = ROOT.parent/'ember-evaluator-private/transfer-cycle-003'
h.PUBLIC = ROOT/'results/codex/transfer-cycle-004'
h.PRIVATE = ROOT.parent/'ember-evaluator-private/transfer-cycle-004'
PROTOCOL = ROOT/'notes/codex/transfer_cycle_004_inputs.md'
COPIES = [f'g{i:04d}' for i in range(1,23)] + ['g0101','g0102','g0201','g0202','g0203']


def seed(namespace, case):
    return int.from_bytes(hashlib.sha256(f'ember-codex-transfer004/{namespace}/{case}'.encode()).digest()[:8], 'big')
...
def main():
    ...
    cases = [
        dict(case='random_er-96',family='random_er',kind='fresh',params=dict(n=96,p=.10),generation_timeout=30),
        dict(case='barabasi_albert-96',family='barabasi_albert',kind='fresh',params=dict(n=96,m=4),generation_timeout=30),
        dict(case='regular-96',family='regular',kind='fresh',params=dict(n=96,d=8),generation_timeout=30),
        dict(case='watts_strogatz-96',family='watts_strogatz',kind='fresh',params=dict(n=96,k=6,beta=.2),generation_timeout=30),
        dict(case='sbm-96',family='sbm',kind='fresh',params=dict(n=96,n_communities=4,p_in=.20,p_out=.02),generation_timeout=30),
        dict(case='random_planar-96',family='random_planar',kind='fresh',params=dict(n=96),generation_timeout=300),
        dict(case='singleton_control-96',family='diagnostic_control',kind='singleton_control',n=96,generation_timeout=30),
        dict(case='branch_control-96',family='diagnostic_control',kind='branch_control',n=96,generation_timeout=30)]
    ...
    entries = {row['graph']: (row,'transfer003',OLD_PUBLIC,OLD_PRIVATE)
               for row in json.loads((OLD_PUBLIC/'panel.json').read_text())['inputs']}
    rows=[]
    for key in COPIES:
    ...
    ledger=[]
    for i,value in enumerate(values,301):
    ...
    if parent['status']=='READY':
        relabel=h.publish(parent,'g0309',parent['case']['case']+'/relabel',parent='g0302')
        relabel['kind']='relabel';rows.append(relabel)
    else:ledger.append(dict(case='barabasi_albert-96/relabel',status='RELABEL_PARENT_UNAVAILABLE'))
    h.save(h.PUBLIC/'attempt_ledger.json',ledger)
    h.save(h.PUBLIC/'panel.json',dict(inputs=rows,realized_inputs=len(rows),intended_structures=33,
        intended_relabelings=3,private_witnesses_shipped=False,generation_plan_sha256=h.sha(h.PUBLIC/'generation_plan.json')))
