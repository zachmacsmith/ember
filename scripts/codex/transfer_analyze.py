"""Independently validate transfer001 outputs against evaluator-only originals.
...
def main(track, archive, output):
    ...
    frozen=bound(PANEL/('freeze-'+track)/'freeze_result.json')
    ...
    assert digest(manifest['tasks'])==frozen['ordered_tasks_sha256'] and len(set(manifest['tasks']))==66
    ...
    entries={entry['graph']:entry for entry in panel['inputs']}
    candidate=frozen['args']['methods'].split(',')[0]
    methods=[candidate,'native-branched-path','mm'];seen=set()
    for tid in manifest['tasks']:
        ...
        assert task['seed']==0 and task['timeout']==60.0 and task['config']=={} and task['method'] in methods
    ...
    for name,entry in entries.items():
        comparison=dict(graph=name,case=entry['case'],family=entry['family'],kind=entry['kind'],parent=entry['parent'])
        for method in methods:comparison[method]=by_key[name,method]
        for ref,label in [('native-branched-path','retained'),('mm','mm')]:
    ...
    signal=len(gains)>=2 and len(families)>=2
    if track=='a':signal=signal and not lost and not worse
    else:signal=signal and any(c['q_delta_mm'] is not None and c['q_delta_mm']<=0 for c in gains)
    totals={}
    ...
    summary=dict(status='PASS' if not errors else 'AUDIT_ERROR',errors=errors,track=track,candidate=candidate,
        totals=totals,continuation_signal=bool(signal) and not errors and not unknown_times,
        ...
        generalization_scope='Development only;20 primary structures plus2 nested relabelings; no confirmation',
    ...
    print(json.dumps({k:summary[k] for k in ('status','track','continuation_signal','fresh_q_gain_graphs','lost_retained_successes','retained_q_regressions','analysis_wall')}))
    if errors: raise SystemExit(1)


if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('track',choices=['a','b','c']);parser.add_argument('archive',type=Path);parser.add_argument('output',type=Path)
    args=parser.parse_args();main(args.track,args.archive.resolve(),args.output.resolve())
