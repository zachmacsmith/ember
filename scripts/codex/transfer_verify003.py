"""Check Transfer003 identities with existing graph/oracle tools, no embedders."""
import hashlib
...
BASE=ROOT/'results/codex/transfer-cycle-003'
PRIVATE=ROOT.parent/'ember-evaluator-private/transfer-cycle-003'
...
def main():
    ...
    panel=bound(BASE/'panel.json');assert panel['realized_inputs']==27
    expected={f'g{i:04d}' for i in range(1,23)} | {'g0101','g0102','g0201','g0202','g0203'}
    assert len(panel['inputs'])==27 and {r['graph'] for r in panel['inputs']}==expected
    assert sum(r['parent'] is not None for r in panel['inputs'])==2
    ledger=bound(BASE/'attempt_ledger.json');assert len(ledger)==3 and all(r['status']=='READY' for r in ledger)
    ...
    spec=importlib.util.spec_from_file_location('_unchanged_oracle003',oraclepath)
