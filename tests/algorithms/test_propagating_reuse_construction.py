...
front = load('frontier', 'packages/ember-qc/src/ember_qc/algorithms/factored/propagating_reuse_construction.py')
pilot = load('audited_pilot', 'scripts/codex/pilot.py')
reference = load('reference', 'packages/ember-qc/src/ember_qc/algorithms/factored/propagating_construction.py')


class FrontierChecks(unittest.TestCase):
    def test_exact_choices_and_counted_work_on_nonbinding_fixtures(self):
        cases = [(nx.path_graph(8), nx.path_graph(14)),
        ...
        for source, target in cases:
            ...
            old = reference.frontier_embed(source, target, timeout=5)
            new = front.frontier_embed(source, target, timeout=5)
            self.assertEqual(old['status'], 'SUCCESS', old['diag'])
            self.assertEqual(new['status'], old['status'], new['diag'])
            self.assertEqual(new['embedding'], old['embedding'])
            for key in ('construction_order', 'committed_updates', 'promotions'):
                self.assertEqual(new['diag'][key], old['diag'][key])
            d = new['diag']; hits += d['domain_reuse_hits']
            self.assertEqual(d['domain_build_calls']+d['domain_reuse_hits'], len(old['diag']['propagation_records']))
            self.assertEqual(old['diag']['scan_count']-d['scan_count'],
                             sum(x['origin_units'] for x in d['domain_reuse_records']))
            self.assertIsNone(pilot.verify_embedding(new['embedding'], source, target))
        self.assertGreater(hits, 0)

    def test_partial_pass_is_never_carried(self):
        source = nx.cycle_graph(6)
        target = nx.convert_node_labels_to_integers(nx.grid_2d_graph(5, 5))
        with patch.object(reference, 'PROPAGATION_CALL', 1), patch.object(front, 'PROPAGATION_CALL', 1):
            old = reference.frontier_embed(source, target, timeout=5)
            new = front.frontier_embed(source, target, timeout=5)
        self.assertEqual(new['embedding'], old['embedding'])
        self.assertEqual(new['diag']['domain_reuse_hits'], 0)
        self.assertTrue(any(r['status']=='propagation_limit' for r in new['diag']['propagation_records']))
        self.assertEqual(new['diag']['scan_count'], old['diag']['scan_count'])

    def test_promotion_and_tree_paths_invalidate_carry(self):
        ...
        self.assertTrue(any(p['reason']=='value_trials' for p in diag['promotions']))
        self.assertGreater(diag['insertion_queries'], 0)
        promoted = {p['vertex'] for p in diag['promotions']}
        self.assertTrue(all(r['after_vertex'] not in promoted for r in diag['domain_reuse_records']))
        self.assertGreater(diag['domain_build_calls'], diag['singleton_trials'])
